...
## MODEL NAMING CONVENTION + gauge_ellipse_v1.0 / gauge_keypoint_v1.0 (2026-08-05)

### Naming convention

Friendly deployment names are `<gauge>_<stage>_v<major>.<minor>`:

- `gauge_ellipse_v1.0` - the face-localizer (ellipse) stage.
- `gauge_keypoint_v1.0` - the center/tip heatmap stage.
- Bump the **minor** for a retrain/re-export that keeps the SAME contract
  (input size + quantization, output layout, decode constants): re-package,
  re-flash, re-verify parity - no firmware code changes.
- Bump the **major** for any contract change (input size, quantization
  convention, output layout/order, or the decode math): the firmware fill
  code, decode constants, and the memory-layout gate must all be revisited.

Concrete mapping (the real artifact/package names behind the friendly names):

| Friendly | Training artifact (WSL ml/artifacts) | stedgeai N6 package (firmware/stm32/n657/st_ai_output/packages) | Flash slot | Firmware wrapper |
|---|---|---|---|---|
| gauge_ellipse_v1.0 | ellipse_iter8_universal_wide_deep (model_int8.tflite) | ellipse_iter8_universal_wide_deep_int8_n6_npu | 0x70400000 | ai_network_mobilenetv2_center_detector.c |
| gauge_keypoint_v1.0 | keypoint_unet_224g_wide_aug (model_int8.tflite) | keypoint_unet_224g_wide_aug_int8_n6_npu | 0x70800000 | ai_network_mobilenetv2_rectifier_hardcase_finetune.c |

### gauge_ellipse_v1.0 (ellipse_iter8_universal_wide_deep_int8)

- **Architecture**: 384x384 grayscale input, multiscale "universal" decoder
  with three resolution heads (24x24, 48x48, 96x96) that vote on the gauge
  ellipse (center x/y + radius x/y) and a 12-value geometry + 3 confidence
  contract. Conv channels [24,32,48,64,96,128]. Trained with
  scripts/train_ellipse_multiscale_universal_384.py (30050 samples incl.
  tiny repeats x200 and board repeats x4); the contract constants
  (CONFIDENCE_OFFSET, GEOMETRY_OFFSET, MAP_VALUES, SIZES) are imported by
  scripts/pipeline_ellipse_keypoint_temperature.py and mirrored in
  app_ai_runtime_tail.inc.
- **Training/export**: QAT to int8 (artifacts hold model_qat.weights.h5 +
  model_int8.tflite). Contract: input q = round(gray*255) - 128 (scale 1/255,
  zp -128), endpoint-mapped 640->384 resize (out = in * (N-1)/(M-1)), output
  dequant scale 1/256, zp -128. On-board decode: argmax head confidence,
  4th-power weighted center on the selected head map, rx/ry from
  GEOMETRY_OFFSET + 4*head.
- **Deployment**: packaged by scripts/package_board_geometry_models_for_n6.py
  -> st_ai_output/packages/ellipse_iter8_universal_wide_deep_int8_n6_npu
  (xSPI2.raw weight image flashed at 0x70400000; flash_boot.ps1 + the
  16-byte signature arrays in ai_network_mobilenetv2_center_detector.c,
  regenerated by tools/extract_model_signature.py). The compiled-in reloc
  image links its GOT at 0x34099400 and its pools around 0x34100000+
  (npuRAM3/4) and 0x342E0000 (npuRAM6).

### gauge_keypoint_v1.0 (keypoint_unet_224g_wide_aug_int8)

- **Architecture**: 224x224x1 U-Net (alpha 1.5, ~2.24M params, int8 ~2.24 MB)
  -> 56x56x2 center/tip heatmaps (channel 0 = center/pivot, channel 1 =
  needle tip). int8 eval: center MAE 3.66 px, tip MAE 6.28 px.
- **Training/export**: QAT (artifacts hold model_fp32.keras, model_qat.keras,
  model_int8.tflite). Same quantization convention as the ellipse (scale
  1/255, zp -128). The crop is a 1.35x square around the ellipse, floored to
  integer source pixels, resized 224x224 with pixel-center bilinear mapping
  ((x+0.5)*in/out - 0.5).
- **Deployment**: same packaging path -> keypoint_unet_224g_wide_aug_int8_n6_npu
  (xSPI2.raw at 0x70800000; signatures in
  ai_network_mobilenetv2_rectifier_hardcase_finetune.c). Heatmap decode:
  dequant (q+128)/256, 4th-power weighting above the 0.03 floor, side-1 (55)
  denominators, then crop-local -> source mapping and the north-zero angle
  (atan2(dx, -dy)).

### Model update procedure (do this in order)

1. Retrain/QAT in WSL (keep the same input size + quantization unless the
   contract intentionally changes). Place the int8 tflite under
   ml/artifacts/<friendly-name>/model_int8.tflite.
2. Package for N6: scripts/package_board_geometry_models_for_n6.py (stages
   under firmware/stm32/n657/st_ai_output/packages/).
3. Run the layout gate: python firmware/stm32/n657/tools/check_model_layout.py
   <package-dir> - FAIL on any tensor base inside a reserved region (see the
   prevention checklist above); adjust the firmware memory layout FIRST if it
   fails, never the other way around.
4. Regenerate the firmware signature arrays with tools/extract_model_signature.py
   and update the wrapper .c files (flash slot stays 0x70400000 / 0x70800000).
5. Flash the weight blobs + app (flash_boot.ps1), then verify ON-TARGET with
   the parity procedure: enable APP_AI_DIAG_DUMP_CENTER_TIP_TENSORS + verbose
   logs, capture one frame, and byte-compare diag_ct_in_000.bin against the
   offline repro of the saved gray8 (scripts/pipeline_ellipse_keypoint_
   temperature.py + the fill-repro helper). The crop sigs and the angle must
   match the offline evaluator on identical pixels.
6. Swap the gauge profile (angle->value) in app_baseline_runtime.c
