...
- The STM32 N657 CubeIDE app makefile was once pointed at a stale AI runtime folder name
  ...
  actually lives under `firmware/stm32/n657/st_ai_output/packages/scalar_full_finetune_from_best_piecewise_calibrated_int8/st_ai_ws/build_scalar_full_finetune_from_best_piecewise_calibrated_int8/`.
...
- After every script that runs in WSL, shut WSL down again with `wsl --shutdown` so the next run starts from a clean session and stale GPU/runtime state does not carry over.
...
- The rectifier v3 and rectified-scalar v2 board assets are now packaged and aligned for the board runtime. The rectifier export/package flow refreshed `firmware/stm32/n657/st_ai_output/atonbuf.rectifier.xSPI2.raw`, and the rectified-scalar package flow refreshed `firmware/stm32/n657/st_ai_output/atonbuf.xSPI2.raw`. The firmware include paths and makefile targets already point at `mobilenetv2_rectifier_hardcase_finetune_v3` and `mobilenetv2_rectified_scalar_finetune_v2`, so the two-stage STM32 path now has matching packaged assets on disk.
...
- A new SD-card import landed a batch of unlabeled board-camera captures in `ml/data/captured_images/` dated `2026-04-20` and `2026-04-22` (raw `.yuv422` files). These are the current candidate images for rectifier inspection/retraining, but they still need preview generation and label triage before we treat them as trustworthy training data.
...
- Whether the scalar model blob in flash actually matches `firmware/stm32/n657/st_ai_output/atonbuf.xSPI2.raw` — the xSPI2 signature check log line `[AI] xSPI2 stage image already present.` was NOT seen in the boot log, meaning the firmware may be skipping the verify step or the verify is not logging
...
**Most likely root cause:** The three-file sync is broken — `ai_network_mobilenetv2_scalar_hardcase_warmstart_int8.c` `#include` or `makefile.targets` paths still point to a different package than `firmware/stm32/n657/st_ai_output/atonbuf.xSPI2.raw`. This exact bug was documented in the "Model Update Process" section and produces a constant non-zero output regardless of input.
...
**Update after inspection:** the scalar wrapper include, `makefile.targets`, and the `firmware/stm32/n657/st_ai_output/atonbuf.xSPI2.raw` start/tail signatures all matched the current `scalar_full_finetune_from_best_piecewise_calibrated_int8` package. That makes stale package wiring much less likely as the root cause; the remaining suspect is the runtime xSPI2/load path or stale on-board flash state.
...
3. **ST Edge AI packaging**: `ml/scripts/run_board_package_prod_model_v0_2_raw_int8.sh`
   ...
   - Output package dir: `firmware/stm32/n657/st_ai_output/packages/prod_model_v0.2_raw_int8/`
   - C source: `firmware/stm32/n657/st_ai_output/packages/prod_model_v0.2_raw_int8/st_ai_output/scalar_full_finetune_from_best_piecewise_calibrated_int8.c`
   - Object files: `firmware/stm32/n657/st_ai_output/packages/prod_model_v0.2_raw_int8/st_ai_ws/build_scalar_full_finetune_from_best_piecewise_calibrated_int8/*.o`
   - xSPI2 blob: `firmware/stm32/n657/st_ai_output/packages/prod_model_v0.2_raw_int8/st_ai_output/scalar_full_finetune_from_best_piecewise_calibrated_int8_atonbuf.xSPI2.raw`
   - Canonical blob: copied to `firmware/stm32/n657/st_ai_output/atonbuf.xSPI2.raw`
...
- **Rectifier**: `mobilenetv2_rectifier_hardcase_finetune_v3`
  ...
  - Packaged blob: `firmware/stm32/n657/st_ai_output/atonbuf.rectifier.xSPI2.raw` (~121 KB, flashed at `0x70600000`)
...
- **Scalar**: `prod_model_v0.2_raw_int8` ← **current best, updated 2026-04-19**
  ...
  - Packaged blob: `firmware/stm32/n657/st_ai_output/atonbuf.xSPI2.raw` (~3.07 MB, flashed at `0x70200000`)
  - Package dir: `firmware/stm32/n657/st_ai_output/packages/prod_model_v0.2_raw_int8/`
...
I added `ml/data/board_rectified_probe_20260422.csv`, which contains 39 `ml/data/captured_images/_live_rectified_probe/**/board_crop.png` samples. These are the closest thing we have to the deployed scalar input domain because they are already the rectified board crops, not the full raw frames.
...
The worker now also waits for the 3-sample history to fill before it emits a baseline temperature. That keeps the first one or two accepted frames quiet so a warm-up glitch does not leak a misleading value like `2.5C` at a true `33C` setpoint.

The AI UART trace now prints the raw model value, the calibrated board-facing value, and the calibration delta for every inference. That makes it much easier to tell whether a bad board reading came from the network itself or from the postprocess fit.

The live AI value is now burst-smoothed across up to 3 frames, with a 2-frame warm-up average and a 3-frame median once the burst is full. That should calm down the board readout without hiding the raw per-frame trace we need for calibration work.

### Image cleanup worker (2026-04-22)

A dedicated ThreadX cleanup worker now runs roughly every 10 minutes and prunes `captured_images` so we keep only the newest image in each 10-minute bucket. The goal is to keep the archive useful for calibration while preventing duplicate captures from filling the SD card with near-identical frames.

The policy is intentionally conservative:

- keep the newest capture per 10-minute window for files older than 24 hours
- keep every capture from the last 24 hours for diagnostics
- delete older images from the same window
- skip files that do not look like capture images or do not have a usable timestamp
- skip pruning entirely if the live RTC read is unavailable or outside the valid timestamp window
- skip the whole prune pass if the DS3231 RTC reports year 2000, because that is the power-on default and should be treated as "clock not valid yet"

That should preserve one representative image from each period while still shedding the bulk of the redundant storage cost.

### Gauge architecture research (2026-04-22)

The current deployed board model is still a lightweight scalar regressor on a 224x224 crop, which is a good embedded compromise but not the latest research direction for analog gauge reading.

The recent literature trend is clear:

- 2021 VDN treats the pointer as a vector and uses a confidence map plus scalar vector components for real-time meter reading.
- 2024 WACV uses a two-stage CNN with segmentation plus landmark regression, trained heavily on synthetic data.
- 2024 ICRA "Under pressure" uses an interpretable multi-stage pipeline with detection, segmentation, and keypoints, and reports less than 2% relative reading error.
- 2025 Measurement uses YOLOv8S plus MC-DeeplabV3Plus with a flexible human-like reading sequence, MobileNetV2 backbone, and explicit angle computation.
- 2025 DialBench/MRLM moves toward a foundation-model style reader with physical relation injection.
- 2026 work pushes even further toward oriented object detection with YOLOv8 plus OBB extraction and angular computation, removing segmentation and OCR entirely.

So the short verdict is:

- A plain scalar head is supported as an efficient deployment compromise.
- If we want to chase the best architectures in the literature, we should move toward explicit geometry: detection, segmentation or OBB extraction, keypoints, then deterministic angle/value conversion.
- The current best offline candidate in this repo already follows that spirit with a rectifier plus scalar chain, which is closer to the literature than the raw scalar model, but still not as explicit as the newest papers.

### Long-term MobileNetV2 geometry split (2026-04-22)

I added a dedicated long-term geometry training wrapper, `ml/scripts/run_mobilenetv2_geometry_longterm.sh`, so we can exercise the explicit keypoint-based MobileNetV2 path with a clean train/val/test split instead of another ad hoc fine-tune.

The split is intentionally domain-aware:

- training gets the main CVAT-labelled pool plus `ml/data/full_labelled_plus_board30_valid_with_new5.csv` as an extra broad-sweep training manifest
- validation is pinned to `ml/data/hard_cases_plus_board30_valid_with_new6.csv`
- test is pinned to `ml/data/board_rectified_probe_20260422.csv`

The training CLI now also supports `--test-manifest`, so the board holdout stays fixed instead of being reshuffled into the random test slice. The long-term wrapper uses `mobilenet_v2_geometry` with a 56x56 keypoint heatmap, warm-started from the current geometry detector checkpoint.

### PowerShell WSL launcher note (2026-04-22)

On this Windows install, `wsl -l -v` shows `docker-desktop` as the default distro, so bare `wsl bash ...` calls land in a shell that does not have `/bin/bash`. The repo now has a PowerShell launcher for the long-term geometry run that forces `Ubuntu-24.04` explicitly:

- `ml/scripts/run_mobilenetv2_geometry_longterm.ps1`

I also switched the older `ml/scripts/run_board_export.ps1` launcher to the same explicit Ubuntu target so the other PowerShell entrypoint does not inherit the default-distro problem either.

### Long-term geometry run result (2026-04-22)

The first long-term `mobilenet_v2_geometry` run completed successfully with the pinned split:

- train pool: 341 examples
- validation: 57 fixed hard-case examples
- test: 39 fixed rectified-board probe examples
- total samples seen in the run summary: 352, with 11 out-of-sweep examples dropped

The final test metrics show a mixed result:

- `gauge_value_mae = 6.2439C`
- `gauge_value_rmse = 6.5169C`
- `baseline_mae_mean_predictor = 11.2892C`
- `keypoint_coords_angle_mae_deg = 46.1151`
- `keypoint_coords_mae = 30.2735`
- `keypoint_heatmaps_mae = 0.3904`

My reading is:

- the value head is clearly better than the mean predictor on the test set
- the geometry/keypoint branch is still not learning a strong spatial representation
- the architecture is promising, but it is not yet the explicit geometry model we want for the long term

So this run is a useful baseline, not the final answer. The next model iteration should either strengthen the geometry supervision or move closer to an explicit detector/keypoint formulation instead of relying on the scalar-style value head alone.

### Long-term geometry inspection (2026-04-22)

I added `ml/scripts/inspect_geometry_model_on_manifest.py` and used it on the pinned board probe test manifest to look at per-sample outputs instead of just aggregate metrics.

The important result is that the geometry head is collapsing to a nearly fixed pose:

- the predicted center keypoint stays around `(25.6, 35.1)` in heatmap coordinates
- the predicted tip keypoint stays around `(35.0, 26.0)` in heatmap coordinates
- the center/tip separation stays around `11-13` heatmap pixels
- the derived angle stays around `-44deg` to `-49deg`
- the heatmap peaks are also almost fixed, with keypoint 0 near the lower-left edge and keypoint 1 near the same upper-right region on almost every sample

That explains the test errors:

- board samples labeled `14C` are predicted around `22C` to `23.7C`
- board samples labeled `28C` are also predicted around `22C` to `23.6C`

So the bottleneck is not the final `gauge_value` conversion layer. The value layer is deterministic from the keypoints, and the keypoint branch itself is not learning enough spatial variation. In other words, the model is learning a stable pose prior, not a true image-conditioned geometry reader.

The next model change should focus on the spatial branch: stronger geometry supervision, better keypoint labels, or a more explicit detector/OBB-style formulation that makes the gauge pose harder to collapse.

### Direction-first follow-up (2026-04-22)

I added a new long-term direction recipe, `ml/scripts/run_mobilenetv2_direction_longterm.sh`, so we can test whether the simpler unit-vector head is a better long-term baseline than the collapsed keypoint/softargmax geometry head.

The split matches the geometry long-term run:

- training adds the broad sweep manifest `ml/data/full_labelled_plus_board30_valid_with_new5.csv`
- validation is pinned to `ml/data/hard_cases_plus_board30_valid_with_new6.csv`
- test is pinned to `ml/data/board_rectified_probe_20260422.csv`
