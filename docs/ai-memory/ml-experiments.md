...
- Keep `data/captured/images/` and `board_weak_focus.csv` as the main regression set for this detector work, because those samples expose the board-specific color and geometry failure modes better than the broader hard-case mix.
...
- [2026-04-28] Relaxed confidence thresholds in single_image_baseline.py (MIN_CONFIDENCE 10.0 -> 5.0, MIN_PEAK_RATIO 1.5 -> 1.2), increasing the detection rate on `data/captured/images/` from 21.5% to ~40.5% while maintaining accuracy on successful detections.
