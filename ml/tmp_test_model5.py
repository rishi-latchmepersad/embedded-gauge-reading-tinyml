...
if model.output_names[0] == 'gauge_value':
    y = {'gauge_value': np.random.rand(2, 1).astype(np.float32), 'needle_mask': np.random.rand(2, 224, 224, 1).astype(np.float32)}
else:
    y = {'needle_mask': np.random.rand(2, 224, 224, 1).astype(np.float32), 'gauge_value': np.random.rand(2, 1).astype(np.float32)}
