...
# Model paths
MODELS = {
    "enhanced_multi_scale": PROJECT_ROOT
    / "artifacts/training/enhanced_multi_scale_e60_s42/best_model.keras",
    "prod_v0.3_scalar": PROJECT_ROOT
...
def _compute_metrics(errors: list[tuple[float, float, float]]) -> dict:
    ...
    print(
        f"Cold MAE: {cold_mae:.2f}°C  Low MAE: {low_mae:.2f}°C  Mid MAE: {mid_mae:.2f}°C  Hot MAE: {hot_mae:.2f}°C"
    )
...
def main():
    ...
    if len(results) > 1:
        ...
        for name, r in results.items():
            print(
                f"{name:<30} {r['mae']:>7.2f} {r['max_error']:>7.2f} {r['over_5']:>5d}/{r['count']}"
            )
        print("-" * 60)
