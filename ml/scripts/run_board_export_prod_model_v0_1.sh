...
# Export the production candidate scalar model to board-ready TFLite artifacts.
# The model is staged into WSL-local storage so TensorFlow does not stall on /mnt/d.
REPO_ROOT="/mnt/d/Projects/embedded-gauge-reading-tinyml/ml"
LOG_DIR="${REPO_ROOT}/artifacts/training_logs"
LOG_FILE="${LOG_DIR}/prod_model_v0_1_board_export.log"
MODEL_IN="${MODEL_IN:-artifacts/deployment/prod_model_v0.1/model.keras}"
MANIFEST_IN="${MANIFEST_IN:-data/hard_cases_plus_board30_valid_with_new5.csv}"
OUTPUT_DIR="${OUTPUT_DIR:-artifacts/deployment/prod_model_v0.1_int8}"
WORK_ROOT="${WORK_ROOT:-${HOME}/prod_model_v0_1_board_export}"
...
echo "[WRAPPER] Staging production model into ${WORK_ROOT}."
cp "${REPO_ROOT}/${MODEL_IN}" "${BASE_MODEL_LOCAL}"

cd "${REPO_ROOT}"

echo "[WRAPPER] Starting board export for prod_model_v0.1."
...
echo "[WRAPPER] Evaluating exported int8 model on the expanded board30-valid hard-case set." | tee -a "${LOG_FILE}"
CUDA_VISIBLE_DEVICES="-1" "${POETRY_BIN}" run python -u scripts/eval_scalar_tflite_on_manifest.py \
  --model "${REPO_ROOT}/${OUTPUT_DIR}/model_int8.tflite" \
