"""Package the DS-CNN v4-S heatmap CD model for STM32N6 (on-chip AXISRAM only, no HyperRAM)."""
...
PACKAGE_DIRNAME: str = "heatmap_cd_v4s_80"
TFLITE_MODEL: Path = (
    REPO_ROOT / "ml" / "artifacts" / "heatmap_cd_v4s_80" / "heatmap_cd_int8.tflite"
)
FIRMWARE_PACKAGE: Path = (
    REPO_ROOT / "firmware" / "stm32" / "n657" / "st_ai_output" / "packages" / PACKAGE_DIRNAME
)
STAI_OUTPUT_DIR: Path = FIRMWARE_PACKAGE / "st_ai_output"
STAI_WS_DIR: Path = FIRMWARE_PACKAGE / "st_ai_ws"
STAGING_BUILD: Path = REPO_ROOT / "tmp" / "stedgeai_heatmap_cd_v4s_build"
TEMP_WORKSPACE: Path = REPO_ROOT / "tmp" / "stedgeai_heatmap_cd_v4s_ws"
TEMP_OUTPUT: Path = REPO_ROOT / "tmp" / "stedgeai_heatmap_cd_v4s_out"
...
def main() -> None:
    ...
    Uses a local on-chip-only mpool (no xSPI1/HyperRAM) since the v4-S model
    fits entirely in AXISRAM2-6 (max_onchip_sram_size = 3072 KB)."""
