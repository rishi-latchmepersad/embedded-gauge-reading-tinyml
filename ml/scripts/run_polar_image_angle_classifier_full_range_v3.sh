...
# Train the 2D polar-image angle classifier on the full real-data pool.
#
# This keeps the angle-bin target from the earlier polar voting branch, but
# feeds the network the full polar projection image instead of a collapsed
# 1D angular profile.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOG_DIR="${ROOT_DIR}/artifacts/training_logs"
LOG_FILE="${LOG_DIR}/polar_image_angle_classifier_full_range_v3.log"
...
TMP_DIR="../tmp"
MANIFEST_PATH="${TMP_DIR}/polar_image_angle_classifier_full_range_manifest_v3.csv"
BOARD_PROBE_TRAIN_MANIFEST="${TMP_DIR}/polar_image_angle_classifier_board_probe_train_v3.csv"
BOARD_PROBE_HOLDOUT_MANIFEST="${TMP_DIR}/polar_image_angle_classifier_board_probe_holdout_v3.csv"
...
OUTPUT_DIR="artifacts/training/polar_image_angle_classifier_full_range_v3"

echo "[WRAPPER] Starting polar image angle-classifier full-range fine-tune v3." | tee "${LOG_FILE}"
...
echo "[WRAPPER] Training the polar image angle-classifier..." | tee -a "${LOG_FILE}"
