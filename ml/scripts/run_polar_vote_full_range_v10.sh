...
#
# This v10 pass keeps the angular axis explicit, but swaps in a
# long-tailed-balanced loss so the middle of the dial cannot dominate the rare
# cold and hot end bins. We keep the same broad manifest mix as v8, because the
# issue looks more like class imbalance than missing examples.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOG_DIR="${ROOT_DIR}/artifacts/training_logs"
RUN_SUFFIX="${RUN_SUFFIX:-v10_balanced_softmax}"
TARGET_MODE="${TARGET_MODE:-sweep}"
LOSS_MODE="${LOSS_MODE:-balanced_softmax}"
