"""Train center/tip U-Net with QAT-conditioned LittleGood data.

Uses the multi-head QAT ellipse model for crop conditioning, making the
center/tip model robust to imperfect ellipse predictions.

Architecture: same proven v9 U-Net (24/36/56/96 channels) with balanced
focal loss weights [4.0, 6.0].
"""
...
STUDENT_DATA = ROOT / "data" / "initial_temp_gauge_v1" / "student_conditioned_qat"
OUT = ROOT / "artifacts" / "gauge_center_tip_qat_conditioned_v1"
...
def build_model():
    ...
    return keras.Model(inputs, out, name="gauge_center_tip_qat")
...
def main():
    ...
    # Export
    tflite_path = OUT / "gauge_center_tip_qat_v1_int8.tflite"
