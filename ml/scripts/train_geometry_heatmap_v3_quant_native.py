#!/usr/bin/env python3
"""Train geometry_heatmap_v3 with quantization-native objectives.

The v3 schedule keeps fake int8 output round-trips active from the beginning so
the training objective matches the deployment path more closely than the v2
late-finetune/QAT approach.
"""
...
DEFAULT_OUTPUT_DIR = Path("ml/artifacts/training/geometry_heatmap_v3_quant_native")
...
class OutputNoiseRampCallback(keras.callbacks.Callback):
    ...
    def on_epoch_begin(self, epoch: int, logs: dict[str, Any] | None = None) -> None:
        ...
        print(f"[V3] output_noise_stddev={current:.6f}", flush=True)
...
class GeometryV3QuantNativeModel(keras.Model):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(name="geometry_heatmap_v3_quant_native_model")
...
def _write_markdown_report(
    *,
    ...
    lines = [
        "# Geometry Heatmap v3 Quantization-Native Training",
        "",
...
def _build_model_from_mode(
    *,
    ...
    """Load the v2 checkpoint or build a fresh ImageNet-initialized model."""

    if mode == "v2":
        return load_geometry_heatmap_keras_model(model_path)
    if mode == "imagenet":
        return build_mobilenetv2_geometry_heatmap_v1(backbone_frozen=True)
    raise ValueError(f"Unknown initialization mode: {mode}")


def main() -> None:
    """Train geometry_heatmap_v3 with quantization-native losses."""

    parser = argparse.ArgumentParser(description="Train geometry_heatmap_v3 quant-native")
    parser.add_argument("--initialization-mode", choices=("v2", "imagenet"), default="v2")
    ...
    if args.debug_canonical_validation_contract:
        ...
        report_path = repo_root / "ml" / "reports" / "geometry_heatmap_v3_canonical_trainer_validation_contract.md"
    ...
    frozen_best_path = output_dir / "model_v3_frozen_best.keras"
    frozen_callback = ReplayMetricCallback(
        metric_prefix="v3_replay",
        samples=val_samples,
        inputs=val_x,
        reference_model=reference_model,
        calibration_candidate=calibration_candidate,
    ...
    if needs_unfreeze and args.initialization_mode == "v2":
        ...
        unfrozen_best_path = output_dir / "model_v3_unfrozen_best.keras"
    ...
    output_model_path = output_dir / "model_v3.keras"
    ...
    print(f"[V3] Output model: {output_model_path}", flush=True)
    print(f"[V3] Selected stage: {final_stage}", flush=True)
    print(f"[V3] Val accepted MAE: {final_summary['accepted_mae_c']:.4f} C", flush=True)
    print(f"[V3] Val acceptance rate: {final_summary['acceptance_rate']:.4f}", flush=True)
    print(f"[V3] Val worst accepted error: {final_summary['worst_accepted_error_c']:.4f} C", flush=True)
