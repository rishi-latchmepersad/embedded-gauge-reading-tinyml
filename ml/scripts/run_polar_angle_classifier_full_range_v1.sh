...
# Train the polar-angle classifier on the full real-data pool.
#
# This is the polar voting path we actually want to compare against the
# baseline: the gauge is cropped, projected to polar coordinates, collapsed
# into a 1D angular profile, and trained as an angle-classification problem.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOG_DIR="${ROOT_DIR}/artifacts/training_logs"
LOG_FILE="${LOG_DIR}/polar_angle_classifier_full_range_v1.log"
...
TMP_DIR="../tmp"
MANIFEST_PATH="${TMP_DIR}/polar_angle_classifier_full_range_manifest_v1.csv"
...
OUTPUT_DIR="artifacts/training/polar_angle_classifier_full_range_v1"

echo "[WRAPPER] Starting polar angle-classifier full-range fine-tune v1." | tee "${LOG_FILE}"
...
python -u scripts/train_polar_angle_classifier_manifest.py \
  ...
  --extra-eval-manifest data/board_rectified_probe_20260422.csv \
  2>&1 | tee -a "${LOG_FILE}"
