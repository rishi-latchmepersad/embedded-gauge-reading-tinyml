...
# Fine-tune the geometry-first MobileNetV2 direction model.
#
# This keeps the learning problem aligned with the gauge geometry:
# predict the needle direction first, then convert direction to temperature
# with the calibrated sweep/min/max mapping.
REPO_ROOT="/mnt/d/Projects/embedded-gauge-reading-tinyml/ml"
LOG_DIR="${REPO_ROOT}/artifacts/training_logs"
LOG_FILE="${LOG_DIR}/mobilenetv2_direction_geometry_first.log"
BASE_MODEL_SRC="${REPO_ROOT}/artifacts/training/mobilenetv2_direction_224_full/model.keras"
BASE_MODEL_LOCAL="${HOME}/ml_eval_cache/mobilenetv2_direction_geometry_first.model.keras"
...
echo "[WRAPPER] Starting geometry-first MobileNetV2 direction fine-tune."
...
"${POETRY_BIN}" run python -u scripts/run_training.py \
  ...
  --run-name mobilenetv2_direction_geometry_first \
  2>&1 | tee "${LOG_FILE}"
