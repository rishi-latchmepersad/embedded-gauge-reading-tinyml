"""Evaluate a Keras scalar model with safe Test-Time Augmentation (TTA).

Applies only photometric augmentations that match the training distribution
(no flips/rotations, which break gauge geometry).
"""
...
def tta_augment(img: tf.Tensor) -> list[tf.Tensor]:
    """Generate safe test-time augmentations preserving gauge geometry."""
    imgs = [img]
    # brightness +/- 0.15
    imgs.append(tf.image.adjust_brightness(img, 0.15))
    imgs.append(tf.image.adjust_brightness(img, -0.15))
    # contrast
    imgs.append(tf.image.adjust_contrast(img, 1.15))
    imgs.append(tf.image.adjust_contrast(img, 0.85))
    # gamma dark
    imgs.append(tf.math.pow(img, 1.5))
    # gamma bright
    imgs.append(tf.math.pow(img, 0.75))
    # slight noise
    noise = tf.random.normal(tf.shape(img), mean=0.0, stddev=0.015, dtype=tf.float32)
    imgs.append(tf.clip_by_value(img + noise, 0.0, 1.0))
    return imgs
...
def main():
    ...
    for path, true_val in rows:
        ...
        if args.tta:
            ...
            pred = float(np.median(preds))  # median is robust to outliers
        else:
