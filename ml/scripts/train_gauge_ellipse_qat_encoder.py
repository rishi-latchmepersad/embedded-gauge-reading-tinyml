"""Train a QAT-safe ellipse detector using the custom encoder from models_geometry_v2.

Architecture: QAT-safe encoder (Conv+BN+ReLU only) + GAP + Dense regression.
All layers are tfmot-compatible for clean int8 QAT export.

Key improvements over the bias-only CNN (v9/v10/v11):
  - BatchNorm normalizes activation ranges for stable quantization
  - ReLU (not relu6) is the only activation - fully tfmot QAT supported
  - No Lambda layers, no tf.nn wrappers
  - Clean QAT export without constant-output collapse
"""
...
DEFAULT_WIDTH = 1.5  # width multiplier for encoder
...
def build_model(width_multiplier: float = 1.5) -> keras.Model:
    """Build QAT-safe ellipse regressor.

    Encoder: 5 stages of Conv-BN-ReLU with stride-2 downsampling.
    Head: GAP + Dense(128) + Dense(5, sigmoid) for [cx, cy, rx, ry, conf].

    Peak activation at 224x224: 112x112xC0 where C0 = _scaled_width(32, wm).
    For wm=1.5: C0=48, peak = 112x112x48 = 602 KB int8.
    """
    wm = float(width_multiplier)
    inputs = keras.Input((IMAGE_SIZE, IMAGE_SIZE, 1), name="image")

    # Convert grayscale to 3-channel (for potential pretrained weight reuse)
    x = keras.layers.Concatenate(name="gray_to_rgb")([inputs, inputs, inputs])

    # Stage 1: 224 -> 112
    s1 = _scaled_width(32, wm)
    x = _conv_bn_relu(x, s1, strides=2, name="enc_s1a")
    x = _conv_bn_relu(x, s1, name="enc_s1b")

    # Stage 2: 112 -> 56
    s2 = _scaled_width(48, wm)
    x = _conv_bn_relu(x, s2, strides=2, name="enc_s2a")
    x = _conv_bn_relu(x, s2, name="enc_s2b")

    # Stage 3: 56 -> 28
    s3 = _scaled_width(64, wm)
    x = _conv_bn_relu(x, s3, strides=2, name="enc_s3a")
    x = _conv_bn_relu(x, s3, name="enc_s3b")

    # Stage 4: 28 -> 14
    s4 = _scaled_width(96, wm)
    x = _conv_bn_relu(x, s4, strides=2, name="enc_s4a")
    x = _conv_bn_relu(x, s4, name="enc_s4b")

    # Stage 5: 14 -> 7
    s5 = _scaled_width(128, wm)
    x = _conv_bn_relu(x, s5, strides=2, name="enc_s5a")
    x = _conv_bn_relu(x, s5, name="enc_s5b")
    ...
    return keras.Model(inputs, outputs, name=f"ellipse_qat_encoder_w{wm}")
...
def _export_int8(model, paths, output, max_cal=512):
    def _rep():
        ...
        indices = rng.permutation(len(paths))[:max_cal]
        for idx in indices:
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path(__file__).resolve().parents[1] / "artifacts" / "gauge_ellipse_qat_encoder_v1")
    ...
    parser.add_argument("--width", type=float, default=DEFAULT_WIDTH, help="Encoder width multiplier")
