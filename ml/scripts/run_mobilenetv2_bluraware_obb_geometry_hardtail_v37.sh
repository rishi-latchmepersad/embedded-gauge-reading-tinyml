...
#
# This version uses a warm-start checkpoint that already lives in the WSL
# cache, so we avoid the slow Windows-mounted D: drive entirely.

REPO_ROOT="/mnt/d/Projects/embedded-gauge-reading-tinyml/ml"
POETRY_BIN="${POETRY_BIN:-${HOME}/.local/bin/poetry}"
LOG_DIR="${REPO_ROOT}/artifacts/training_logs"
TRAIN_LOG_FILE="${LOG_DIR}/mobilenetv2_bluraware_obb_geometry_hardtail_v37.log"
RUN_NAME="${RUN_NAME:-mobilenetv2_bluraware_obb_geometry_hardtail_v37}"
INIT_MODEL_LOCAL="${INIT_MODEL_LOCAL:-${HOME}/ml_eval_cache/mobilenetv2_bluraware_obb_relation_geometry_v42.model.keras}"
...
if [[ ! -f "${INIT_MODEL_LOCAL}" ]]; then
  echo "[WRAPPER] Cached warm-start checkpoint is missing: ${INIT_MODEL_LOCAL}" >&2
  exit 1
fi
