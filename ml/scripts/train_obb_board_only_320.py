"""Board-specific OBB localizer at 320x320 colour input.

Problem: The existing OBB model was trained on 341 phone photos + 76 board captures.
Phone photos have the gauge centered in the frame; board captures have it near the
bottom (cy~0.91 in 320x320 space). The model learned "gauge is at frame center" from
the phone data, causing it to predict cy~0.49 on board captures instead of cy~0.91.

Solution: Use PXL phone photos (which have accurate geometry labels) with **positional
augmentation** to synthetically shift the gauge to different positions in the frame,
including the bottom where the board mounting places it. Also include new 320x320 YUV
board captures (June 7, 2026) with the gauge at cy~0.45.

Data sources (priority order for deduplication):
  1. PXL phone photos from manifest — full geometry, positional augmentation
  2. Board captures from manifest — full geometry
  3. 320x320 YUV board captures (June 7, 2026) — labeled center + radius
  4. annotate_30/annotations.csv — manual center labels
  5. annotate_batch2/annotations_batch2.csv — manual center labels
  6. ai_annotated_board_captures.csv — AI board capture labels

Usage:
  cd ml && poetry run python scripts/train_obb_board_only_320.py
"""
...
# Fixed gauge geometry for board captures — inner dial radius as a fraction
# of the source frame side length (224 px for standard board captures).
...
# Positional augmentation: shift gauge to random cy positions in this range.
# This teaches the model to find the gauge at any position, including the bottom
# of the frame where the board mounting places it (cy~0.91).
...
def _load_fullframe_obb_data_colour(
    ...
    Handles both standard image formats (PNG, JPG) and YUV422 raw files.
    """
    # Check if this is a YUV file
    is_yuv = tf.strings.regex_full_match(image_path, ".*\\.yuv422$")

    def _load_yuv():
        """Load YUV422 file and convert to RGB."""
        # Read raw bytes
        raw = tf.io.read_file(image_path)
        # Decode YUYV format: Y0 U Y1 V per 2 pixels
        # Reshape to (320, 640) then extract channels
...
def _is_original_capture(filename: str) -> bool:
    ...
    Filters out .gray.png, _preview, _yuy2, and _glare_ variants that were
    created for diagnostic / display purposes and should not be training samples.
    """
...
def _obb_params_from_center_224(
    ...
    The dial radius is derived from the fixed gauge geometry ratio so that
    the w/h outputs are reasonable even without a manual radius label.
    """
...
def _load_manifest_examples(
    ...
    These rows have full geometry labels (center + radius), so they are the
    highest-quality source.  Returns the number of examples added.
    """
    added = 0
    with open(MANIFEST_PATH, newline="", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            # Only board captures (not phone photos)
...
def _load_pxl_photo_examples(
    ...
    These are high-resolution images (3472×4624) with accurate center + radius
    labels. The gauge is typically centered in the frame (cy~0.5 at 320×320).
    Positional augmentation will shift the gauge to different positions during
    training to teach the model to find it anywhere in the frame.
    """
    added = 0
    with open(MANIFEST_PATH, newline="", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            # Only PXL photos (not board captures)
...
def _load_annotate_csv(
    ...
    *,
    ...
    Expected columns: ``filename, cx, cy`` (pixel coords in *source_size* space).
    Rows whose image file does not exist on disk are silently skipped.
    """
    added = 0
    with open(csv_path, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            examples.append(
                TrainingExample(
                    image_path=fpath,
                    value=0.0,  # temperature not used by OBB localizer
...
def _load_ai_annotated_examples(
    ...
    The CSV has columns ``image_path, center_x, center_y`` where *image_path*
    is relative to ``ml/data/`` and centers are in 224 px space.
    """
    added = 0
...
def _load_yuv422_image(yuv_path: str) -> np.ndarray:
    ...
    YUV422 format: Y0 U Y1 V per 2 pixels, so 320x320 = 204800 bytes.
    Returns RGB array in [0, 255] uint8 format.
    """
...
def _load_yuv_board_captures(
    ...
    These are real board captures from June 7, 2026, with the gauge at
    cy~0.45 in 320x320 space. Labels are in the companion CSV file.
    """
    added = 0
    ...
    with open(YUV_LABELS_CSV, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            # Compute OBB params for 320x320 canvas
            obb_params = _compute_fullframe_obb_params(
                320, 320,
...
def _build_board_only_examples() -> list[TrainingExample]:
    ...
    Sources are loaded in priority order so that higher-quality labels win
    when the same image appears in multiple CSVs:
      1. PXL phone photos from manifest (full geometry — primary training data)
      2. Board captures from manifest (full geometry)
      3. 320x320 YUV board captures (June 7, 2026 — real board mounting)
      4. annotate_30/annotations.csv (manual center labels)
      5. annotate_batch2/annotations_batch2.csv (manual center labels)
      6. ai_annotated_board_captures.csv (AI board capture labels)

    PXL photos have the gauge at cy~0.5; positional augmentation will shift
    it to cover the full range including cy~0.91 (board mounting position).
    """
    examples: list[TrainingExample] = []
    seen: set[str] = set()
...
def main() -> None:
    """Train board-specific OBB model."""
    ...
    # Build examples from board captures only
    print("\n[1/5] Loading board capture examples from all sources...")
    examples = _build_board_only_examples()

    if len(examples) < 20:
        print("ERROR: Not enough board captures to train. Need at least 20.")
        return
    ...
    # Compute edge weights for temperature-based sample weighting
...
def _augment_obb_positional(
    ...
    This teaches the model to find the gauge at any position in the frame,
    not just where it appears in the training data.

    Args:
        image: Preprocessed image tensor [H, W, 3] in [0, 1]
        obb_params: Original OBB params [cx, cy, w, h, cos2t, sin2t]
        cy_min, cy_max: Range for random cy position
        cx_min, cx_max: Range for random cx position

    Returns:
        Augmented image and updated OBB params
    """
    img_h = tf.cast(tf.shape(image)[0], tf.float32)
    img_w = tf.cast(tf.shape(image)[1], tf.float32)

    # Extract original center (in normalized coords)
    # obb_params shape: [6]
    ...
    # Compute translation in pixels
    shift_x = (target_cx - orig_cx) * img_w
    shift_y = (target_cy - orig_cy) * img_h

    # Apply translation to image using affine transform
    # Translation matrix in projective transform format: [a0, a1, a2, b0, b1, b2, c0, c1]
    # For pure translation: [1, 0, tx, 0, 1, ty, 0, 0]
    transforms = tf.stack([
        1.0, 0.0, -shift_x,
        0.0, 1.0, -shift_y,
        0.0, 0.0,
    ...
    # Update OBB params with new center
    obb_aug = tf.stack([
        target_cx,
        target_cy,
        obb_params[2],  # width unchanged
        obb_params[3],  # height unchanged
        obb_params[4],  # cos2t unchanged
        obb_params[5],  # sin2t unchanged
    ])
...
def _augment_obb(
    ...
    Positional augmentation shifts the gauge to a random position,
    teaching the model to find it anywhere in the frame.
    Photometric augmentation simulates lighting variations.
    """
...
def main() -> None:
    ...
    # Compute center error in pixels @320
    ...
    tflite_path = run_dir / "obb_board_only_int8.tflite"
    ...
    print("\n=== Done ===")
    print(f"Next steps:")
    print(f"  1. Package for Cube.AI: ml/scripts/package_obb_board_only_for_n6.py")
    print(f"  2. Update firmware signatures in app_ai.c")
    print(f"  3. Flash and test on board")
