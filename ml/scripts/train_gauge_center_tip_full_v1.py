"""Train center/tip U-Net with ALL available data (7,793 images).

Uses the full student_conditioned dataset (484 LittleGood images including
board_captures_2 and test_3) plus the generic gauge corpus.

Architecture: v9 U-Net (24/36/56/96 channels) with balanced focal loss.
"""
...
STUDENT_DATA = ROOT / "data" / "center_tip_full_pipeline_v1"
OUT = ROOT / "artifacts" / "gauge_center_tip_full_pipeline_v2"
...
def build_model():
    ...
    return keras.Model(inputs, out, name="gauge_center_tip_full")
...
def main() -> None:
    """Train, quantize, export, and evaluate the universal keypoint model."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--fp32-epochs", type=int, default=FP32_EPOCHS)
    ...
    # Use only the full student_conditioned data (includes LittleGood + board_captures_2 + test_3)
    # Do NOT also load TEMP_DATA to avoid double-counting LittleGood images
    student = {s: np.load(STUDENT_DATA / f"{s}.npz") for s in ("train", "val", "test")}

    x_train = np.concatenate((xb, student["train"]["inputs"]))
    y_train = np.concatenate((yb, student["train"]["heatmaps"]))
    x_val = np.concatenate((xv, student["val"]["inputs"]))
    y_val = np.concatenate((yv, student["val"]["heatmaps"]))
    ...
    # FP32
    model = build_model()
    lr = WarmupCosineDecay(FP32_LR, steps * args.fp32_epochs, steps * 2)
    model.compile(optimizer=keras.optimizers.Adam(lr), loss=focal_heatmap_loss)
    model.fit(make_dataset(x_train, y_train, True), validation_data=make_dataset(x_val, y_val, False), epochs=args.fp32_epochs, verbose=2)
    # why: preserve the expensive FP32 stage if QAT or export fails later.
    model.save(output / "model_fp32.keras")

    # QAT
    qat = tfmot.quantization.keras.quantize_model(model)
    qat_lr = WarmupCosineDecay(QAT_LR, steps * args.qat_epochs, steps)
    qat.compile(optimizer=keras.optimizers.Adam(qat_lr), loss=focal_heatmap_loss)
    qat.fit(make_dataset(x_train, y_train, True), validation_data=make_dataset(x_val, y_val, False), epochs=args.qat_epochs, verbose=2)

    # Export
    tflite_path = output / "gauge_center_tip_full_v1_int8.tflite"
    ...
    errors = np.linalg.norm((decoded - student["test"]["points"]) * INPUT_SIZE, axis=2)

    c_t_pred = decoded[:, 1] - decoded[:, 0]
    c_t_gt = student["test"]["points"][:, 1] - student["test"]["points"][:, 0]
    ...
    report = {
        "model": "gauge_center_tip_full_v1",
