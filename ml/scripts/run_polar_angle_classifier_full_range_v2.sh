...
# Train the polar-angle classifier on the full real-data pool, while also
# folding in the live board probe captures as an additional domain source.
#
# The board probe set is split into a train slice and a held-out slice so we
# can adapt to the live board appearance without losing a clean evaluation set.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOG_DIR="${ROOT_DIR}/artifacts/training_logs"
LOG_FILE="${LOG_DIR}/polar_angle_classifier_full_range_v2.log"
...
TMP_DIR="../tmp"
MANIFEST_PATH="${TMP_DIR}/polar_angle_classifier_full_range_manifest_v2.csv"
BOARD_PROBE_TRAIN_MANIFEST="${TMP_DIR}/polar_angle_classifier_board_probe_train_v2.csv"
BOARD_PROBE_HOLDOUT_MANIFEST="${TMP_DIR}/polar_angle_classifier_board_probe_holdout_v2.csv"
...
OUTPUT_DIR="artifacts/training/polar_angle_classifier_full_range_v2"

echo "[WRAPPER] Starting polar angle-classifier full-range fine-tune v2." | tee "${LOG_FILE}"
...
with manifest_path.open("w", encoding="utf-8", newline="") as handle:
    ...
    count, rows = _iter_rows(
        SourceManifest(board_probe_train_path, default_weight=1.00)
    )
    kept = 0
    for row in rows:
        image_path = row["image_path"]
...
echo "[WRAPPER] Training the polar angle-classifier..." | tee -a "${LOG_FILE}"
