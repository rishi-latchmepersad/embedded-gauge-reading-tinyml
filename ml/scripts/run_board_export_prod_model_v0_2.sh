...
# Export the current prod-v0.2 candidate scalar model to board-ready TFLite artifacts.
# The model is staged into WSL-local storage so TensorFlow does not stall on /mnt/d.
REPO_ROOT="/mnt/d/Projects/embedded-gauge-reading-tinyml/ml"
LOG_DIR="${REPO_ROOT}/artifacts/training_logs"
LOG_FILE="${LOG_DIR}/prod_model_v0_2_board_export.log"
...
OUTPUT_DIR="${OUTPUT_DIR:-artifacts/deployment/prod_model_v0.2_raw_int8}"
WORK_ROOT="${WORK_ROOT:-${HOME}/prod_model_v0_2_board_export}"
...
echo "[WRAPPER] Staging prod-v0.2 model into ${WORK_ROOT}."
cp "${REPO_ROOT}/${MODEL_IN}" "${BASE_MODEL_LOCAL}"

cd "${REPO_ROOT}"

echo "[WRAPPER] Starting board export for prod_model_v0.2."
