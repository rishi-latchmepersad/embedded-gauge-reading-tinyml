"""Package the QARepVGG-Pro heatmap OBB model for STM32N6 (on-chip, no HyperRAM).

Single-model replacement for the OBB + centre-detector pipeline.
Outputs 3 tensors:
  heatmap [1,40,40,1]  — logits → sigmoid → parabolic peak → gauge centre (cx, cy)
  box_size [1,40,40,2] — (w, h) at peak cell → OBB box dimensions
  angle    [1,40,40,2] — tanh → (sin2θ, cos2θ) at peak cell → gauge rotation

Centre decoding uses 1-D parabolic Taylor refinement around the argmax,
achieving ~0.1–0.3 px sub-pixel accuracy on the STM32 CPU.

Usage (Windows, Cube.AI 10.2.0 environment):
...
MODEL_NAME: str = "qarepvgg_pro_a175_int8"  # name kept for firmware compat (model is α=1.25)
PACKAGE_DIRNAME: str = "qarepvgg_pro_a175_int8"
TFLITE_MODEL: Path = (
    REPO_ROOT
    / "ml"
    / "artifacts"
    / "training"
    / "qat_qarepvgg_pro_a1.25_20260614_153729"
    / "qarepvgg_pro_a1.25_int8.tflite"
)
...
STAGING_BUILD: Path = REPO_ROOT / "tmp" / "stedgeai_qarepvgg_pro_build"
TEMP_WORKSPACE: Path = REPO_ROOT / "tmp" / "stedgeai_qarepvgg_pro_ws"
TEMP_OUTPUT: Path = REPO_ROOT / "tmp" / "stedgeai_qarepvgg_pro_out"
...
def main() -> None:
    ...
    for path in (
        TFLITE_MODEL, stedgeai_exe, pack_python, npu_driver,
        LOCAL_MPOOL, neural_art_profile,
    ):
        if not path.is_file():
    ...
    for path in (STAGING_BUILD, TEMP_WORKSPACE, TEMP_OUTPUT):
        if path.exists():
            shutil.rmtree(path)
    for path in (
        STAI_OUTPUT_DIR, STAI_WS_DIR,
        STAGING_BUILD, TEMP_WORKSPACE, TEMP_OUTPUT,
    ):
    ...
    generate_cmd = [
        ...
        "--memory-pool", _to_windows_path(LOCAL_MPOOL),
        "--st-neural-art", f"n657-onchip@{_to_windows_path(neural_art_profile)}",
    ...
    # Step 2: npu_driver relocatable build
    npu_cmd = [
        str(pack_python),
    ...
    print(
        f"[PACKAGE] raw blob: {generated_raw} "
    ...
    # Copy raw blob to firmware package
    shutil.copy2(generated_raw, EXPECTED_XSPI2_RAW)
    print(
        f"[PACKAGE]   {generated_raw.name} -> {EXPECTED_XSPI2_RAW}", flush=True,
    )
