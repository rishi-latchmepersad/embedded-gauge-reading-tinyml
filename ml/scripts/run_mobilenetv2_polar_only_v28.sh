...
# Train the new single-input polar MobileNetV2 regression model on all real
# sources plus corrected synthetic gauge renders.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOG_DIR="${ROOT_DIR}/artifacts/training_logs"
LOG_FILE="${LOG_DIR}/mobilenetv2_polar_only_v28.log"
...
COMBINED_MANIFEST="${TMP_DIR}/combined_polar_only_manifest_v28.csv"
...
OUTPUT_DIR="artifacts/training/mobilenetv2_polar_only_v28"
HARD_CASE_MANIFEST="data/hard_cases_plus_board30_valid_with_new5.csv"

echo "[WRAPPER] Starting polar-only v28." | tee "${LOG_FILE}"
...
echo "[WRAPPER] Building the combined polar-only manifest..." | tee -a "${LOG_FILE}"
...
"""Build a weighted real + synthetic manifest for the polar-only run."""
...
echo "[WRAPPER] Training the new polar-only model..." | tee -a "${LOG_FILE}"
