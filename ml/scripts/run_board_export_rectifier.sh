...
# Export the production rectifier model to board-ready TFLite artifacts.
# The model is staged into WSL-local storage so TensorFlow does not stall on /mnt/d.
REPO_ROOT="/mnt/d/Projects/embedded-gauge-reading-tinyml/ml"
LOG_DIR="${REPO_ROOT}/artifacts/training_logs"
LOG_FILE="${LOG_DIR}/mobilenetv2_rectifier_v3_board_export.log"
MODEL_IN="${MODEL_IN:-artifacts/training/mobilenetv2_rectifier_hardcase_finetune_v3/model.keras}"
MANIFEST_IN="${MANIFEST_IN:-data/hard_cases_plus_board30_valid_with_new5.csv}"
OUTPUT_DIR="${OUTPUT_DIR:-artifacts/deployment/mobilenetv2_rectifier_hardcase_finetune_v3_int8}"
WORK_ROOT="${WORK_ROOT:-${HOME}/mobilenetv2_rectifier_v3_board_export}"
...
echo "[WRAPPER] Staging rectifier model into ${WORK_ROOT}."
cp "${REPO_ROOT}/${MODEL_IN}" "${BASE_MODEL_LOCAL}"

cd "${REPO_ROOT}"

echo "[WRAPPER] Starting board export for rectifier v3."
