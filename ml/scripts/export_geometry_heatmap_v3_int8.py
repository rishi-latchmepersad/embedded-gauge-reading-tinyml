#!/usr/bin/env python3
"""Export the geometry_heatmap_v3 quant-native model to TFLite."""
...
DEFAULT_MODEL_PATH = Path("ml/artifacts/training/geometry_heatmap_v3_quant_native/best_model.keras")
...
DEFAULT_OUTPUT_DIR = Path("ml/artifacts/deployment/geometry_heatmap_v3_tflite")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Export geometry_heatmap_v3 TFLite artifacts")
    ...
    print(f"[V3 EXPORT] Loading v3 Keras model from {model_path}", flush=True)
    ...
    print(f"[V3 EXPORT] Using {len(train_examples)} clean train rows as representative data.", flush=True)
    ...
    print("[V3 EXPORT] Converting float32 TFLite model...", flush=True)
    ...
    float32_path = output_dir / "model_v3_float32.tflite"
    float32_path.write_bytes(float32_tflite)

    print("[V3 EXPORT] Converting full int8 TFLite model...", flush=True)
    ...
    int8_path = output_dir / "model_v3_int8.tflite"
    ...
    print(f"[V3 EXPORT] Wrote {config_path}", flush=True)
    print(f"[V3 EXPORT] Float32 model: {float32_path}", flush=True)
    print(f"[V3 EXPORT] Int8 model: {int8_path}", flush=True)
