...
#
# This reuses the harder synthetic checkpoint so the real fine-tune starts from
# a backbone that has already seen low-contrast, tail-heavy gauge-like frames.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOG_DIR="${ROOT_DIR}/artifacts/training_logs"
LOG_FILE="${LOG_DIR}/mobilenetv2_rectified_scalar_from_hard_synth_v13.log"
...
OUTPUT_DIR="artifacts/training/mobilenetv2_rectified_scalar_from_hard_synth_v13"

echo "[WRAPPER] Starting rectified fine-tune from hard synthetic pretrain v13." | tee "${LOG_FILE}"
