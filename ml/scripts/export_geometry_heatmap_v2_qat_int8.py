#!/usr/bin/env python3
"""Export the quantization-noise fine-tuned geometry_heatmap_v2 model to TFLite."""
...
DEFAULT_MODEL_PATH = Path("ml/artifacts/training/geometry_heatmap_v2_qat/model_qat.keras")
...
DEFAULT_OUTPUT_DIR = Path("ml/artifacts/deployment/geometry_heatmap_v2_qat_tflite")
...
def _write_representative_manifest(
    *,
    ...
    for index, example in enumerate(examples):
        ...
        jitter_example = load_heatmap_sample(
            example,
            base_path,
            input_size=input_size,
            heatmap_size=heatmap_size,
        ...
        rows.append(
            {
                ...
                "source_width": int(example.source_width),
                "source_height": int(example.source_height),
                "crop_x1": int(jitter_example.metadata["crop_x1"]),
                "crop_y1": int(jitter_example.metadata["crop_y1"]),
                "crop_x2": int(jitter_example.metadata["crop_x2"]),
                "crop_y2": int(jitter_example.metadata["crop_y2"]),
                "crop_width": int(jitter_example.metadata["crop_width"]),
                "crop_height": int(jitter_example.metadata["crop_height"]),
                "center_x_224": float(jitter_example.metadata["center_x_224"]),
                "center_y_224": float(jitter_example.metadata["center_y_224"]),
                "tip_x_224": float(jitter_example.metadata["tip_x_224"]),
                "tip_y_224": float(jitter_example.metadata["tip_y_224"]),
                "preprocessing_mode": SELECTED_PREPROCESSING_MODE,
                "resize_method": "bilinear",
                "channel_strategy": "rgb",
                "normalization": "uint8_to_float32_0_1",
...
def main() -> None:
    """Export the QAT-fine-tuned model to float32 and int8 TFLite artifacts."""

    parser = argparse.ArgumentParser(description="Export geometry_heatmap_v2 QAT TFLite artifacts")
    ...
    print(f"[QAT EXPORT] Loading QAT Keras model from {model_path}", flush=True)
    ...
    print(f"[QAT EXPORT] Using {len(train_examples)} clean train rows as representative data.", flush=True)
    ...
    print("[QAT EXPORT] Converting float32 TFLite model...", flush=True)
    ...
    float32_path = output_dir / "model_qat_float32.tflite"
    float32_path.write_bytes(float32_tflite)

    print("[QAT EXPORT] Converting full int8 TFLite model...", flush=True)
    ...
    int8_path = output_dir / "model_qat_int8.tflite"
    ...
    config: dict[str, Any] = {
        ...
        "notes": [
            "The representative dataset is train-only and includes both identity and mild-jitter board crops.",
            "TFLite emits the raw heatmap outputs in reverse semantic order, so consumers must reorder them before decode.",
    ...
    print(f"[QAT EXPORT] Wrote {config_path}", flush=True)
    print(f"[QAT EXPORT] Float32 model: {float32_path}", flush=True)
    print(f"[QAT EXPORT] Int8 model: {int8_path}", flush=True)
