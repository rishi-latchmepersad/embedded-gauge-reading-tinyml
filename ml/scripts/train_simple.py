...
def main():
    parser = argparse.ArgumentParser(
        description="Train gauge reader with simple supervised learning"
    )
    parser.add_argument(
        "--train-manifest",
        type=Path,
        required=True,
        help="CSV with image_path,value columns",
    )
    parser.add_argument(
        "--val-manifest", type=Path, default=None, help="Optional validation CSV"
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ML_ROOT / "artifacts" / "training" / "simple_supervised",
    )
