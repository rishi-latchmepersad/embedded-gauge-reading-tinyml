"""MobileNetV2 joint OBB + center localizer at 320x320.

Trains a single MobileNetV2 model that predicts both the oriented bounding
box of the gauge face and the needle pivot center in the same forward pass.
This replaces the previous two-stage pipeline (OBB model + separate center
detector) with one model, saving NPU memory and inference time.

Transfer-learning from ImageNet via MobileNetV2 provides strong initial
features, while positional augmentation teaches the model to find the gauge
at any position in the frame -- critical because phone photos have the gauge
centered (~cy=0.5) but the board mount puts it near the bottom (~cy=0.85-0.91).

Target: <2.5 MB int8 TFLite, <10 px center MAE at 320x320.

Usage:
  cd ml && poetry run python scripts/train_obb_center_mobilenetv2.py
"""
...
# Import TF first, configure immediately, then import Keras
...
IMAGE_WIDTH: int = 320
SEED: int = 42
BATCH_SIZE: int = 12  # α=0.75 fills more GPU memory
EPOCHS: int = 120
EPOCHS_FINETUNE: int = 0  # Disabled — fine-tuning always regresses on this dataset
LEARNING_RATE: float = 1e-3
VAL_FRACTION: float = 0.20
MOBILENET_ALPHA: float = 0.75
HEAD_UNITS_1: int = 384
HEAD_UNITS_2: int = 160
HEAD_DROPOUT_1: float = 0.30
HEAD_DROPOUT_2: float = 0.20
CENTER_LOSS_WEIGHT: float = 3.0  # Push harder on center accuracy

# Positional augmentation: shift the gauge to random positions so the model
# learns to find it anywhere in the frame, especially at board-mount positions.
import os as _os
IMAGE_HEIGHT = int(_os.environ.get("IMAGE_HEIGHT", IMAGE_HEIGHT))
IMAGE_WIDTH = int(_os.environ.get("IMAGE_WIDTH", IMAGE_WIDTH))
SEED = int(_os.environ.get("SEED", SEED))
BATCH_SIZE = int(_os.environ.get("BATCH_SIZE", BATCH_SIZE))
EPOCHS = int(_os.environ.get("EPOCHS", EPOCHS))
EPOCHS_FINETUNE = int(_os.environ.get("EPOCHS_FINETUNE", EPOCHS_FINETUNE))
LEARNING_RATE = float(_os.environ.get("LEARNING_RATE", LEARNING_RATE))
VAL_FRACTION = float(_os.environ.get("VAL_FRACTION", VAL_FRACTION))
MOBILENET_ALPHA = float(_os.environ.get("MOBILENET_ALPHA", MOBILENET_ALPHA))
HEAD_UNITS_1 = int(_os.environ.get("HEAD_UNITS_1", HEAD_UNITS_1))
HEAD_UNITS_2 = int(_os.environ.get("HEAD_UNITS_2", HEAD_UNITS_2))
HEAD_DROPOUT_1 = float(_os.environ.get("HEAD_DROPOUT_1", HEAD_DROPOUT_1))
HEAD_DROPOUT_2 = float(_os.environ.get("HEAD_DROPOUT_2", HEAD_DROPOUT_2))
CENTER_LOSS_WEIGHT = float(_os.environ.get("CENTER_LOSS_WEIGHT", CENTER_LOSS_WEIGHT))
del _os
...
def _preprocess_colour(
    ...
    Nearest-neighbor resize preserves dial geometry.
    Normalises output to [0, 1].
    """
...
def _load_fullframe_obb_data_colour(
    ...
    The center_xy is the first two components of obb_params (the gauge center
    is the same as the needle pivot for our single-gauge setup).
    """
    # Check if this is a YUV file
    is_yuv = tf.strings.regex_full_match(image_path, ".*\\.yuv422$")

    def _load_yuv():
        """Load YUV422 (YUYV) file and convert to RGB."""
    ...
    # Center target = first two elements of OBB params
    ...
    return (
        ...
        {
            "obb_params": sample_weight,
            "center_xy": sample_weight * CENTER_LOSS_WEIGHT,
        },
    )
...
def _load_manifest_examples(
    ...
    with open(MANIFEST_PATH, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            # Remap to 320x320 pre-upscaled directory (JPGs converted to PNG)
...
def _load_pxl_photo_examples(
    ...
    These are high-res (3472x4624) with accurate full geometry labels.
    Positional augmentation will shift the gauge during training.
    """
    added = 0
    with open(MANIFEST_PATH, newline="", encoding="utf-8") as f:
        reader = csv.DictReader(f)
    ...
    with open(AI_CENTERS_CSV, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if not _is_original_capture(rel_path):
                continue
            # Remap to 320x320 pre-upscaled directory
...
def _build_all_examples() -> list[TrainingExample]:
    ...
    Sources loaded in priority order (highest quality first):
      1. PXL phone photos (full geometry, 3472x4624)
      2. Board captures from manifest (full geometry)
      3. YUV422 board captures (320x320, June 7 2026)
      4. annotate_30 (manual center labels)
      5. annotate_batch2 (manual center labels)
      6. AI annotated centers (auto-labeled)
    """
    examples: list[TrainingExample] = []
    seen: set[str] = set()
...
def _augment_positional(
    ...
    Updates both the image (via projective transform) and the OBB params
    (center components set to the new target position).  This is critical
    for teaching the model to find the gauge at board-mount positions
    (cy~0.85-0.91) as well as phone-photo positions (cy~0.5).

    Only the center components of obb_params are updated; w, h, and angle
    are unchanged by pure translation.
    """
    ...
    target_cx = tf.random.uniform(
        [], POSITIONAL_AUG_CX_MIN, POSITIONAL_AUG_CX_MAX
    )
    target_cy = tf.random.uniform(
        [], POSITIONAL_AUG_CY_MIN, POSITIONAL_AUG_CY_MAX
    )

    # Compute pixel translation
    shift_x = (target_cx - orig_cx) * img_w
    shift_y = (target_cy - orig_cy) * img_h

    # Projective-transform matrix for pure translation:
    # [1, 0, -tx, 0, 1, -ty, 0, 0]
    ...
    # Update only the center values in the OBB params
    obb_aug = tf.stack([
        target_cx,
        target_cy,
        obb_params[2],
...
def _augment_photometric(image: tf.Tensor) -> tf.Tensor:
    """Apply photometric-only augmentation (strong, no spatial changes).

    Uses the strong augmentation settings that proved best in the
    board-mimic ablation (varB): wider brightness/contrast ranges
    to cover the board camera's exposure variation.
    """
    # Brightness: +/- 30% (strong — covers underexposed board captures)
    image = tf.image.random_brightness(image, max_delta=0.30)
    # Contrast: 0.5 - 1.5 (strong — covers low-contrast board captures)
    image = tf.image.random_contrast(image, lower=0.50, upper=1.50)
    # Saturation: 0.75 - 1.25
    image = tf.image.random_saturation(image, lower=0.75, upper=1.25)
    image = tf.clip_by_value(image, 0.0, 1.0)
    # Small random hue shift
    image = tf.image.random_hue(image, max_delta=0.05)
    image = tf.clip_by_value(image, 0.0, 1.0)
    return image


_ROTATION_MAX_RAD: float = 0.05  # ≈ ±2.9°
...
def _augment_rotation(
    ...
    Rotates the image and updates the OBB center and angle labels
    accordingly.  Teaches the model to handle slight camera tilt.
    """
    angle = tf.random.uniform([], -_ROTATION_MAX_RAD, _ROTATION_MAX_RAD)
    cos_a = tf.cos(angle)
    sin_a = tf.sin(angle)

    # Rotation matrix elements for projective transform around (0.5, 0.5)
    a0 = cos_a
    a1 = -sin_a
    ...
    # Update OBB center via rotation around (0.5, 0.5)
    cx = obb_params[0]
    cy = obb_params[1]
    dx = cx - 0.5
    dy = cy - 0.5
    ...
    # Update OBB angle (cos(2(θ+α)), sin(2(θ+α)))
    cos2t = obb_params[4]
    sin2t = obb_params[5]
...
def _augment_train(
    ...
    Positional augmentation is applied first so the OBB and center targets
    stay consistent with the image content.  Rotation is applied second (also
    label-consistent), then photometric-only augmentation last.
    """
    # 1) Positional augmentation — shift gauge, update OBB + center targets
    image, new_obb = _augment_positional(image, y["obb_params"])

    # 2) Rotation augmentation — small random rotation with label update
    image, new_obb = _augment_rotation(image, new_obb)

    new_center = new_obb[:2]  # Center = first two OBB params
    y_aug = {"obb_params": new_obb, "center_xy": new_center}

    # 3) Photometric augmentation — lighting variations only
...
def main() -> None:
    """Train MobileNetV2 OBB + center model on all available data."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_name = f"obb_center_mnv2_{timestamp}"
    run_dir = ARTIFACTS_DIR / run_name
    run_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 60)
    print("MobileNetV2 + CoordConv Joint OBB + Center Localizer")
    print(f"  alpha={MOBILENET_ALPHA}, {IMAGE_HEIGHT}x{IMAGE_WIDTH}, "
          f"head={HEAD_UNITS_1}-{HEAD_UNITS_2}, center_wt={CENTER_LOSS_WEIGHT}")
    print(f"  CoordConv + rotation aug (±{_ROTATION_MAX_RAD:.2f} rad) + "
          f"2-phase train")
    print(f"  Run: {run_dir}")
    print("=" * 60)
    ...
    # --- 2. Split ---
    print("\n[2/6] Splitting data (stratified random)...")
    ...
    train_cy = [ex.obb_params[1] for ex in train_exs]
    print(f"  Train cy: [{min(train_cy):.3f}, {max(train_cy):.3f}], "
          f"mean={np.mean(train_cy):.3f}")
    ...
    def make_dataset(
        ...
        if shuffle:
            ds = ds.shuffle(
                len(exs), seed=SEED, reshuffle_each_iteration=True
            )
        ds = ds.map(
        ...
        if augment:
            ds = ds.map(
                _augment_train, num_parallel_calls=4,
            )
    ...
    # --- 4. Build model ---
    print("\n[4/6] Building MobileNetV2 dual-head model...")
    model = build_mobilenetv2_obb_center_model(
        image_height=IMAGE_HEIGHT,
        image_width=IMAGE_WIDTH,
        alpha=MOBILENET_ALPHA,
        head_units_1=HEAD_UNITS_1,
        head_units_2=HEAD_UNITS_2,
        head_dropout_1=HEAD_DROPOUT_1,
        head_dropout_2=HEAD_DROPOUT_2,
        pretrained=True,
    ...
    model.compile(
        optimizer=keras.optimizers.Adam(
            learning_rate=keras.optimizers.schedules.CosineDecay(
                ...
                decay_steps=EPOCHS * max(1, len(train_exs) // BATCH_SIZE),
                alpha=1e-4,  # Final LR = initial * alpha
            ),
            clipnorm=1.0,
        ),
        loss={
            "obb_params": HuberLoss(delta=0.05, name="obb_huber"),
            "center_xy": HuberLoss(delta=0.02, name="center_huber"),
        },
        loss_weights={
            "obb_params": 1.0,
        ...
        metrics={
            "obb_params": [keras.metrics.MeanAbsoluteError(name="mae")],
    ...
    # --- 5. Train ---
    print(f"\n[5/6] Training ({EPOCHS} epochs, frozen backbone, cosine LR)...")
    callbacks = [
    ...
    # Center error in pixels at 320x320
    cx_err = (
        np.mean(np.abs(test_center_preds[:, 0] - test_center_true[:, 0]))
        * IMAGE_WIDTH
    )
    cy_err = (
        np.mean(np.abs(test_center_preds[:, 1] - test_center_true[:, 1]))
        * IMAGE_HEIGHT
    )
    ...
    def representative_dataset():
        for ex in test_exs[:50]:
            # Skip YUV files — tf.io.decode_image cannot handle raw YUV
    ...
    tflite_path = run_dir / "obb_center_mnv2_int8.tflite"
    ...
    print(f"Next: package for Cube.AI + update firmware signatures.")
