...
#
# This run keeps the OBB-aware backbone, upweights the known hard cases, and
# then evaluates the resulting geometry reader as the scalar stage in an OBB ->
# crop -> reader cascade.

REPO_ROOT="/mnt/d/Projects/embedded-gauge-reading-tinyml/ml"
POETRY_BIN="${POETRY_BIN:-${HOME}/.local/bin/poetry}"
LOG_DIR="${REPO_ROOT}/artifacts/training_logs"
TRAIN_LOG_FILE="${LOG_DIR}/mobilenetv2_bluraware_obb_geometry_hardtail_v35.log"
RUN_NAME="${RUN_NAME:-mobilenetv2_bluraware_obb_geometry_hardtail_v35}"
...
mkdir -p "${LOG_DIR}"
mkdir -p "$(dirname "${INIT_MODEL_SRC}")"
...
echo "[WRAPPER] Init model: ${INIT_MODEL_SRC}" | tee -a "${TRAIN_LOG_FILE}"
