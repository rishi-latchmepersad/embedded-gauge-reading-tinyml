...
ML = REPO / "ml"
CAPS = REPO / "data" / "captured" / "images"
IMAGE_SIZE = 224
    ...
    for row in reader:
        img_col = next(
            (
                k
                for k in row
                if "image" in k.lower() or "path" in k.lower() or "file" in k.lower()
            ),
            None,
        )
        if img_col is None:
...
print(
    f"Board 17:xx captures below gate: {sum(1 for m in board_means if m < 130)}/{len(board_means)}"
)
