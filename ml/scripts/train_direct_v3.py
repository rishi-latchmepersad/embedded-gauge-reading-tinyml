#!/usr/bin/env python3
"""
Direct gauge reader v3 — MobileNetV2 tiny (alpha=0.35) with stronger augmentation.

Builds on v2's 8.01°C board MAE breakthrough.
New:
1. Random translation (crop shift up to ±16px) to simulate luma crop variation
2. Random Gaussian blur to simulate focus issues
3. Stronger dropout (0.5)
4. Cosine LR decay
5. Higher board weight (15x)
6. Longer training (150 epochs, patience=30)
"""
...
def augment_image(image, label):
    """Stronger augmentation: color jitter, noise, blur, translation."""
    rgb = image[..., :3]
    ...
    # Random translation (simulates crop position variation)
    ...
    # Zero out wrapped edges (few pixels of border)
    rgb = rgb[:216, :216, :]
...
def main():
    ...
    print("=" * 60)
    print(f"DIRECT GAUGE READER v3 — MobileNetV2 tiny + stronger aug")
    print(f"  Dropout: {conf.dropout}  Board weight: {conf.board_weight}x  LR: {conf.lr}")
    print(f"  Augment: {not args.no_augment} (color+noise+translate+blur)")
