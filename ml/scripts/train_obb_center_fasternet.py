"""FasterNet-T0/T1 joint OBB + center localizer at 320x320.

FasterNet uses Partial Convolutions (PConv) for efficient feature extraction.
This script trains a FasterNet-style backbone with dual OBB and center heads.

Target: <2.5 MB int8 TFLite, <10 px center MAE at 320x320.

Usage:
  cd ml && poetry run python scripts/train_obb_center_fasternet.py
"""
...
def build_fasternet_obb_center_model(
    image_height: int = 320,
    image_width: int = 320,
    *,
    variant: str = "T0",
    ...
    """Build FasterNet-style backbone with dual OBB + center heads.

    FasterNet uses Partial Convolutions (PConv) for efficiency.
    T0: smaller, T1: larger.
    """
    ...
    # FasterNet backbone configuration
    if variant == "T0":
    ...
    # Stem
    x = keras.layers.Conv2D(channels[0], 3, strides=2, padding="same", use_bias=False)(x)
    x = keras.layers.BatchNormalization()(x)
    x = keras.layers.Activation("relu")(x)

    # Stages with Partial Convolutions (approximated with grouped conv)
    for stage_idx, (ch, depth) in enumerate(zip(channels[1:], depths[1:])):
        for block_idx in range(depth):
            stride = 2 if block_idx == 0 else 1
            shortcut = x
            # Grouped conv (simulates PConv)
            x = keras.layers.Conv2D(ch, 3, strides=stride, padding="same", groups=4, use_bias=False)(x)
            x = keras.layers.BatchNormalization()(x)
            x = keras.layers.Activation("relu")(x)
            # Pointwise
            x = keras.layers.Conv2D(ch, 1, use_bias=False)(x)
            x = keras.layers.BatchNormalization()(x)
            if stride > 1 or shortcut.shape[-1] != ch:
                shortcut = keras.layers.Conv2D(ch, 1, strides=stride, use_bias=False)(shortcut)
    ...
    # FC layers
    x = keras.layers.Dense(channels[-1] * 2)(x)
    x = keras.layers.Activation("relu")(x)
    ...
    model = keras.Model(
        ...
        name=f"fasternet_{variant}_obb_center_h{head_units_1}-{head_units_2}",
    )
    return model
...
def _augment_positional(image: tf.Tensor, obb_params: tf.Tensor) -> tuple[tf.Tensor, tf.Tensor]:
    ...
    image_aug = tf.raw_ops.ImageProjectiveTransformV3(
        images=image[tf.newaxis, ...], transforms=transforms[tf.newaxis, :],
...
def _augment_rotation(image: tf.Tensor, obb_params: tf.Tensor) -> tuple[tf.Tensor, tf.Tensor]:
    ...
    obb_rot = tf.stack([
        ...
        obb_params[2], obb_params[3], new_cos2t, new_sin2t,
    ])
...
def main() -> None:
    """Train FasterNet OBB + center model."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_name = f"obb_center_fasternet_{timestamp}"
    run_dir = ARTIFACTS_DIR / run_name
    run_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 60)
    print("FasterNet-T0 + CoordConv Joint OBB + Center Localizer")
    print(f"  variant=T0, {IMAGE_HEIGHT}x{IMAGE_WIDTH}, head={HEAD_UNITS_1}-{HEAD_UNITS_2}")
    print(f"  Run: {run_dir}")
    print("=" * 60)
    ...
    print("\n[4/6] Building FasterNet model...")
    model = build_fasternet_obb_center_model(
        image_height=IMAGE_HEIGHT, image_width=IMAGE_WIDTH,
        variant="T0", head_units_1=HEAD_UNITS_1, head_units_2=HEAD_UNITS_2,
    ...
    tflite_path = run_dir / "obb_center_fasternet_int8.tflite"
    ...
    summary = {
        "run_name": run_name, "model": "FasterNet-T0 + CoordConv",
