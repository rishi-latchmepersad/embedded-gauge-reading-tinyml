...
#
# This variant copies the warm-start checkpoint into the WSL home cache before
# loading it. The cached path avoids the slow Windows-mounted D: drive and
# keeps the initialization step from stalling for minutes.

REPO_ROOT="/mnt/d/Projects/embedded-gauge-reading-tinyml/ml"
POETRY_BIN="${POETRY_BIN:-${HOME}/.local/bin/poetry}"
LOG_DIR="${REPO_ROOT}/artifacts/training_logs"
TRAIN_LOG_FILE="${LOG_DIR}/mobilenetv2_bluraware_obb_geometry_hardtail_v36.log"
RUN_NAME="${RUN_NAME:-mobilenetv2_bluraware_obb_geometry_hardtail_v36}"
INIT_MODEL_SRC="${INIT_MODEL_SRC:-${REPO_ROOT}/artifacts/training/mobilenetv2_bluraware_obb_geometry_v34/model.keras}"
INIT_MODEL_LOCAL="${INIT_MODEL_LOCAL:-${HOME}/ml_eval_cache/mobilenetv2_bluraware_obb_geometry_hardtail_v36.model.keras}"
...
if [[ ! -f "${INIT_MODEL_SRC}" ]]; then
  echo "[WRAPPER] Warm-start checkpoint is missing: ${INIT_MODEL_SRC}" >&2
  exit 1
fi
...
echo "[WRAPPER] Starting blur-aware OBB-geometry hard-tail fine-tune." | tee "${TRAIN_LOG_FILE}"
