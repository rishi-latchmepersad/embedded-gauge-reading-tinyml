"""
Train a 320×320 heatmap center detector with MobileNetV3-Small + improved decoder.

Pipeline:
  1. Load 320×320 rectified gauge crops + 80×80 Gaussian heatmap targets
  2. MobileNetV3-Small encoder (alpha=0.75, imagenet, frozen)
  3. Improved decoder: wider convs + residual upsampling → 80×80 heatmap
  4. Train with MSE loss on heatmap
  5. Evaluate with soft-argmax center pixel error
  6. Export TFLite (float32 + int8) with full validation

Usage:
  python scripts/train_heatmap_cd_320.py
"""
...
ARTIFACT_DIR = Path(__file__).resolve().parents[1] / "artifacts" / "heatmap_cd_320"
...
def ds_conv_block(
    ...
    if shortcut.shape[-1] == filters and stride == 1:
...
def build_model(
    ...
    """MobileNetV3-Small encoder + Decoder with DS conv blocks.

    Encoder: MobileNetV3-S (α=0.75, frozen, ImageNet), output 10×10×C.
    Decoder: DS conv blocks at each upsampled resolution → 80×80 heatmap.
    """
    ...
    x = backbone.output  # (10, 10, C) — C=120 for alpha=0.75

    # Decoder with DS conv blocks
    x = layers.UpSampling2D(2, interpolation="bilinear", name="dec_up1")(x)  # 20×20
    x = ds_conv_block(x, 64, stride=1, name="dec_ds1")

    x = layers.UpSampling2D(2, interpolation="bilinear", name="dec_up2")(x)  # 40×40
    x = ds_conv_block(x, 32, stride=1, name="dec_ds2")

    x = layers.UpSampling2D(2, interpolation="bilinear", name="dec_up3")(x)  # 80×80
    x = ds_conv_block(x, 16, stride=1, name="dec_ds3")
...
def eval_tflite(
    tflite_path: str, X_val: np.ndarray, y_val: np.ndarray, label: str
) -> dict:
...
def main() -> None:
    ...
    fp32_err = eval_tflite(
        str(ARTIFACT_DIR / "heatmap_cd_fp32.tflite"), X_val, y_val, "TFLite fp32"
    )
    int8_err = eval_tflite(
        str(ARTIFACT_DIR / "heatmap_cd_int8.tflite"), X_val, y_val, "TFLite int8"
    )
