"""
DS-CNN v3b: CoordConv + wider encoder + MSE loss + data augmentation.

Architecture same as v3:
  - CoordConv input (x, y coordinate channels)
  - Encoder: 48→96→192→384
  - Decoder with skip connections
  - MSE loss (not focal — focal hurt v3)
  - Online augmentation: brightness, contrast, hue, saturation

Usage:
  python scripts/train_heatmap_cd_ds_v3b.py
"""
...
ARTIFACT_DIR = Path(__file__).resolve().parents[1] / "artifacts" / "heatmap_cd_ds_v3b"
...
def build_model(
    ...
    """DS-CNN v3b: CoordConv + wider encoder + skip connections."""
    ...
    x = layers.ReLU(name="enc_head_relu")(x)
...
def augment_image(img: tf.Tensor, hm: tf.Tensor) -> tuple[tf.Tensor, tf.Tensor]:
    """Photometric augmentation: brightness, contrast, hue, saturation."""
...
def main() -> None:
    ...
    results = {
        "model": "depthwise_separable_cnn_v3b_aug",
        "params": trainable,
