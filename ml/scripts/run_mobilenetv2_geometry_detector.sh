...
# Fine-tune the geometry-first MobileNetV2 model on the clean labels.
#
# This run keeps the paper-aligned structure:
# - predict the gauge keypoint geometry
# - derive the gauge value from the detected coordinates
# - let hard cases reinforce the same geometry-to-value mapping
REPO_ROOT="/mnt/d/Projects/embedded-gauge-reading-tinyml/ml"
LOG_DIR="${REPO_ROOT}/artifacts/training_logs"
LOG_FILE="${LOG_DIR}/mobilenetv2_geometry_detector.log"
BASE_MODEL_SRC="${REPO_ROOT}/artifacts/training/mobilenetv2_detector_geometry/model.keras"
BASE_MODEL_LOCAL="${HOME}/ml_eval_cache/mobilenetv2_geometry_detector.model.keras"
...
echo "[WRAPPER] Starting geometry-first MobileNetV2 fine-tune."
...
"${POETRY_BIN}" run python -u scripts/run_training.py \
  ...
  --run-name mobilenetv2_geometry_detector \
  2>&1 | tee "${LOG_FILE}"
