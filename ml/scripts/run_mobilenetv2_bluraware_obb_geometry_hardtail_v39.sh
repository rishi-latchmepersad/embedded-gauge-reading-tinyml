...
#
# This variant skips ImageNet pretraining entirely so we can avoid any backbone
# weight downloads or cached-weight stalls inside WSL.

REPO_ROOT="/mnt/d/Projects/embedded-gauge-reading-tinyml/ml"
POETRY_BIN="${POETRY_BIN:-${HOME}/.local/bin/poetry}"
LOG_DIR="${REPO_ROOT}/artifacts/training_logs"
TRAIN_LOG_FILE="${LOG_DIR}/mobilenetv2_bluraware_obb_geometry_hardtail_v39.log"
RUN_NAME="${RUN_NAME:-mobilenetv2_bluraware_obb_geometry_hardtail_v39}"
...
echo "[WRAPPER] Starting blur-aware OBB-geometry hard-tail fine-tune (no pretrain)." | tee "${TRAIN_LOG_FILE}"
