...
# Train a polar voting model with a broad auxiliary head on the full real-data pool.
#
# The sharp head stays the deployable readout, but the broad head gives the
# backbone a less brittle global sweep target so the cold and hot ends do not
# collapse toward the middle as easily.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOG_DIR="${ROOT_DIR}/artifacts/training_logs"
RUN_SUFFIX="${RUN_SUFFIX:-v11_broad_aux}"
TARGET_MODE="${TARGET_MODE:-sweep}"
LOSS_MODE="${LOSS_MODE:-standard}"
AUX_HEAD_MODE="${AUX_HEAD_MODE:-broad}"
AUX_SIGMA_MULTIPLIER="${AUX_SIGMA_MULTIPLIER:-3.0}"
...
echo "[WRAPPER] Loss mode:      ${LOSS_MODE}" | tee -a "${LOG_FILE}"
echo "[WRAPPER] Aux head mode:  ${AUX_HEAD_MODE}" | tee -a "${LOG_FILE}"
echo "[WRAPPER] Aux sigma mult: ${AUX_SIGMA_MULTIPLIER}" | tee -a "${LOG_FILE}"
...
python -u scripts/train_polar_angle_classifier_manifest.py \
  ...
  --loss-mode "${LOSS_MODE}" \
  --aux-head-mode "${AUX_HEAD_MODE}" \
  --aux-sigma-multiplier "${AUX_SIGMA_MULTIPLIER}" \
  --seed 21 \
