...
# Train a blur-aware OBB-relation-geometry MobileNetV2 variant and benchmark
# it against the hard-case manifest using the fixed board front-end.
# This mirrors the literature-backed pattern: localize first, then reason
# about the pointer/scale relation with explicit heatmaps, masks, and landmarks.

REPO_ROOT="/mnt/d/Projects/embedded-gauge-reading-tinyml/ml"
POETRY_BIN="${POETRY_BIN:-${HOME}/.local/bin/poetry}"
LOG_DIR="${REPO_ROOT}/artifacts/training_logs"
TRAIN_LOG_FILE="${LOG_DIR}/mobilenetv2_bluraware_obb_relation_geometry_v42.log"
EVAL_LOG_FILE="${LOG_DIR}/mobilenetv2_bluraware_obb_relation_geometry_v42_hardcase.log"
RUN_NAME="${RUN_NAME:-mobilenetv2_bluraware_obb_relation_geometry_v42}"
INIT_MODEL_LOCAL="${HOME}/ml_eval_cache/mobilenetv2_bluraware_obb_relation_geometry_v42.model.keras"
...
INIT_MODEL_SRC=""
for candidate in \
  "${REPO_ROOT}/artifacts/training/mobilenetv2_bluraware_obb_geometry_v34/model.keras" \
  "${REPO_ROOT}/artifacts/training/mobilenetv2_obb_relation_geometry_v40/model.keras" \
  "${REPO_ROOT}/artifacts/training/mobilenetv2_bluraware_reader_v41/model.keras" \
  "${REPO_ROOT}/artifacts/training/scalar_rectified_crop_finetune_v2_20260422/model.keras"
do
  if [[ -f "${candidate}" ]]; then
    INIT_MODEL_SRC="${candidate}"
    break
  fi
done

if [[ -z "${INIT_MODEL_SRC}" ]]; then
  echo "[WRAPPER] No warm-start checkpoint was found." >&2
  exit 1
fi

cp -f "${INIT_MODEL_SRC}" "${INIT_MODEL_LOCAL}"

cd "${REPO_ROOT}"

echo "[WRAPPER] Starting blur-aware OBB-relation-geometry training." | tee "${TRAIN_LOG_FILE}"
...
RECTIFY_ALL=1 "${POETRY_BIN}" run python -u scripts/run_training.py \
  --run-name "${RUN_NAME}" \
  --model-family mobilenet_v2_obb_relation_geometry \
