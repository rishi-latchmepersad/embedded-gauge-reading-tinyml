...
#
# This v9 pass keeps the angular axis explicit, but it pushes harder on the
# tails so the cold and hot ends stop collapsing toward the middle. We still
# use the full hard-case pool, but we give rare bins more say during training.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOG_DIR="${ROOT_DIR}/artifacts/training_logs"
RUN_SUFFIX="${RUN_SUFFIX:-v9_tail_power}"
TARGET_MODE="${TARGET_MODE:-sweep}"
BIN_WEIGHT_POWER="${BIN_WEIGHT_POWER:-0.85}"
BIN_WEIGHT_CLIP_MAX="${BIN_WEIGHT_CLIP_MAX:-4.00}"
...
echo "[WRAPPER] Bin weight pow: ${BIN_WEIGHT_POWER}" | tee -a "${LOG_FILE}"
echo "[WRAPPER] Bin clip max:   ${BIN_WEIGHT_CLIP_MAX}" | tee -a "${LOG_FILE}"
...
bin_weight_power = float(os.environ["BIN_WEIGHT_POWER"])
bin_weight_clip_max = float(os.environ["BIN_WEIGHT_CLIP_MAX"])
bin_weights = temp_bins.map(
    lambda b: float((median_count / float(bin_counts[b])) ** bin_weight_power)
)
df["sample_weight"] = df["sample_weight"] * bin_weights.astype(np.float32)
df["sample_weight"] = df["sample_weight"].clip(lower=0.5, upper=bin_weight_clip_max)
