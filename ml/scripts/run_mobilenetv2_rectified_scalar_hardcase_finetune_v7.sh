...
# Fine-tune the tiny rectified scalar model on a hard-case-heavy weighted mix.
#
# This run keeps the alpha=0.35 / head=64 backbone from the strict baseline,
# but shifts the training emphasis toward the cold/hot manifests that the
# earlier mixed recipe still underfit.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOG_DIR="${ROOT_DIR}/artifacts/training_logs"
LOG_FILE="${LOG_DIR}/mobilenetv2_rectified_scalar_hardcase_v7.log"
...
TRAIN_MANIFEST="data/rectified_scalar_hardcase_train_v7.csv"
...
echo "[WRAPPER] Starting hard-case rectified scalar fine-tune v7." | tee "${LOG_FILE}"
...
echo "[WRAPPER] Building hard-case-heavy rectified training manifest..." | tee -a "${LOG_FILE}"
...
root = Path.cwd()
out_manifest = root / "data" / "rectified_scalar_hardcase_train_v7.csv"
holdout_manifest = root / "data" / "hard_cases_plus_board30_valid_with_new5.csv"

# Order matters: the earlier rows win if the same image appears in multiple
# sources, so put the most focused manifests first.
...
"${POETRY_BIN}" run python -u scripts/train_all_data_baseline.py \
  --output-dir artifacts/training/mobilenetv2_rectified_scalar_hardcase_v7 \
