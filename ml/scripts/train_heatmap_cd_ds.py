"""
Train a 320×320 heatmap center detector with depthwise-separable CNN + upsampling decoder.

Architecture:
  Encoder: Conv2D → 3× DSConv (stride-2) blocks, 16→32→64→128 channels
  Decoder: 3× UpSampling2D + DSConv blocks, 128→64→32→1 channel heatmap
  ~24K params — trains end-to-end on 282 samples without overfitting

Usage:
  python scripts/train_heatmap_cd_ds.py
"""
...
ARTIFACT_DIR = Path(__file__).resolve().parents[1] / "artifacts" / "heatmap_cd_ds"
...
def build_model(
    ...
    """Custom depthwise-separable CNN encoder + upsampling decoder for heatmap.

    Encoder:  320→160→80→40→20  (4 stages, stride-2 each)
    Decoder:  20→40→80→HEATMAP  (3 upsampling stages)
    """
    inputs = layers.Input(shape=input_shape, name="input")

    # Encoder
    x = layers.Conv2D(16, 3, strides=2, padding="same", use_bias=False, name="enc_conv")(inputs)
    x = layers.BatchNormalization(name="enc_conv_bn")(x)
    x = layers.ReLU(name="enc_conv_relu")(x)  # 160×160×16

    x = ds_conv_block(x, 32, stride=2, name="enc_ds1")   # 80×80×32
    x = ds_conv_block(x, 64, stride=2, name="enc_ds2")   # 40×40×64
    x = ds_conv_block(x, 128, stride=2, name="enc_ds3")  # 20×20×128
    ...
    # Output heatmap (80×80, matches existing dataset targets)
