...
print("Loading luma_crop_detector...", flush=True)
...
class Config:
    ...
    epochs: int = 150
    lr: float = 3e-4
    board_weight: float = 15.0
    val_split: float = 0.15
    dropout: float = 0.5
    experiment: str = "exp1"
    output_dir: str = "/tmp/gauge_direct_v3"
...
def augment_image(image, label):
    ...
    rgb = tf.image.random_hue(rgb, 0.08)
    rgb = tf.clip_by_value(rgb, 0.0, 1.0)
    noise = tf.random.normal(tf.shape(rgb), 0.0, 0.03)
    rgb = tf.clip_by_value(rgb + noise, 0.0, 1.0)
    shifts = tf.random.uniform([2], -16, 17, dtype=tf.int32)
    rgb = tf.roll(rgb, shift=shifts[0], axis=0)
    rgb = tf.roll(rgb, shift=shifts[1], axis=1)
    rgb = tf.image.crop_to_bounding_box(rgb, 4, 4, 216, 216)
...
def main():
    ...
    ap.add_argument("--board-weight", type=float, default=15.0)
    ap.add_argument("--epochs", type=int, default=150)
    ap.add_argument("--dropout", type=float, default=0.5)
    ap.add_argument("--lr", type=float, default=3e-4)
    ap.add_argument("--no-augment", action="store_true")
    ap.add_argument("--experiment", default="exp1")
    ap.add_argument("--output-dir", default="/tmp/gauge_direct_v3")
    args = ap.parse_args()
    ...
    print("=" * 60, flush=True)
    print("DEBUG V3 — MobileNetV2 tiny + stronger aug", flush=True)
    print(f"  Dropout: {conf.dropout}  Board weight: {conf.board_weight}x  LR: {conf.lr}", flush=True)
    ...
    print(f"  Loaded metadata: {len(all_s)} samples", flush=True)
    ...
    print(f"  Total: {len(all_s)} samples ({len(board_s)} board)", flush=True)
    ...
    print(f"  Train (weighted): {len(tr_w)}  Val: {len(va)}", flush=True)

    print("  Loading training images...", flush=True)
    tr_imgs, tr_tgts = [], []
    for i, s in enumerate(tr_w):
        if i % 100 == 0 and i > 0:
            print(f"    loaded {i}/{len(tr_w)}", flush=True)
        img = load_image(Path(s["path"]), s["board"], conf, pp_dir)
        if img is None:
            continue
        tr_imgs.append(img)
        tr_tgts.append(s["temp"])
    print(f"  Training images loaded: {len(tr_imgs)}", flush=True)
    ...
    print(f"  Validation images loaded: {len(va_imgs)}", flush=True)

    print("  Creating TF datasets...", flush=True)
    ...
    hist = model.fit(tr_ds, validation_data=va_ds, epochs=min(conf.epochs, 5), callbacks=cb, verbose=2)
    ...
    print(f"\n  Board MAE: {r['mae']:.2f}°C  med={r['med']:.2f}°C", flush=True)
