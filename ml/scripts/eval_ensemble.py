...
def main():
    ...
    parser.add_argument("--model1", type=Path, required=True)
    parser.add_argument("--model2", type=Path, required=True)
    ...
    for path, true_val in rows:
        ...
        p1 = float(np.asarray(model1.predict(inp[None], verbose=0)).flatten()[0])
        p2 = float(np.asarray(model2.predict(inp[None], verbose=0)).flatten()[0])
        pred = (p1 + p2) / 2.0
        err = pred - true_val
        errors.append((true_val, pred, err, p1, p2))
        print(f"  true={true_val:6.1f}  pred={pred:7.2f}  err={err:+.2f}  m1={p1:7.2f}  m2={p2:7.2f}  {path.name}")
