"""Fast batched calibration for the current best scalar model."""
...
def build_dataset(rows, ml_root, batch_size=32, target_size=224):
    paths = []
    truths = []
    for row in rows:
    ...
    return ds, np.array(truths), paths
...
def main():
    ...
    output_path = PROJECT_ROOT / "tmp" / "calibration_isotonic.json"
    ...
    print(f"[CALIB] {len(rows)} rows in manifest")

    print("[CALIB] Building dataset...")
    ds, truths, valid_paths = build_dataset(rows, PROJECT_ROOT, batch_size=32)
    print(f"[CALIB] {len(valid_paths)} valid images")

    print("[CALIB] Running batched inference...")
    preds = model.predict(ds, verbose=1)
    if isinstance(preds, list):
        preds = preds[0]
    preds = preds.flatten()[:len(truths)]

    mae_before = np.mean(np.abs(truths - preds))
    print(f"[CALIB] Before calibration: MAE = {mae_before:.3f}C")

    # Isotonic regression
    print("[CALIB] Fitting isotonic regression...")
