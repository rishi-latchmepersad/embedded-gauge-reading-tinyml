"""MobileNetV3-Small joint OBB + center localizer at 320x320.

Trains a single MobileNetV3-Small model that predicts both the oriented bounding
box of the gauge face and the needle pivot center in the same forward pass.
MobileNetV3-Small is more efficient than V2 at the same alpha, giving better
accuracy within the 2.5 MB int8 TFLite budget.

Transfer-learning from ImageNet via MobileNetV3-Small provides strong initial
...
Usage:
  cd ml && poetry run python scripts/train_obb_center_mobilenetv3.py
"""
...
IMAGE_WIDTH: int = 320
SEED: int = 42
BATCH_SIZE: int = 16  # V3-Small is lighter than V2 α=0.75, can fit more
EPOCHS: int = 200
...
def _preprocess_colour(
    ...
    Normalises output to [0, 1] (MobileNetV3 expects [0, 1]).
    """
...
def main() -> None:
    """Train MobileNetV3-Small OBB + center model on all available data."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_name = f"obb_center_mnv3_{timestamp}"
    run_dir = ARTIFACTS_DIR / run_name
    run_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 60)
    print("MobileNetV3-Small + CoordConv Joint OBB + Center Localizer")
    ...
    # --- 4. Build model ---
    print("\n[4/6] Building MobileNetV3-Small dual-head model...")
    model = build_mobilenetv3_obb_center_model(
    ...
    tflite_path = run_dir / "obb_center_mnv3_int8.tflite"
