...
SCALAR_NEW = Path("artifacts/training/scalar_rectified_crop_finetune_v1/model.keras")
...
print("Loading new scalar...", flush=True)
scalar_new = tf.keras.models.load_model(str(SCALAR_NEW), compile=False)
print("Loading old scalar...", flush=True)
...
def _rectifier_crop(src, rectifier):
    oh, ow = src.shape[:2]
    ...
    if bw < 0.25 or bh < 0.25 or bw > 0.95 or bh > 0.95:
        ...
        x0 = max(0.0,(cx-0.5*bw)*ow); y0 = max(0.0,(cy-0.5*bh)*oh)
        x1 = min(float(ow),(cx+0.5*bw)*ow); y1 = min(float(oh),(cy+0.5*bh)*oh)
...
# Columns: Old=fixed crop (how it was trained), New=rectifier crop (how new model was trained)
...
for row in rows:
    ...
    try:
        ...
        old_pred = _scalar_run(scalar_old, _fixed_crop(src))
        new_pred = _scalar_run(scalar_new, _rectifier_crop(src, rect))
