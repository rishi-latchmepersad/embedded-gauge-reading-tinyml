...
#
# This v5 pass keeps the angular axis explicit, but it tightens the loss and
# rebalances the manifest across the whole temperature range so cold and hot
# tails both matter. The goal is to keep the classical polar-voting inductive
# bias while reducing the midrange collapse we saw in earlier runs.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOG_DIR="${ROOT_DIR}/artifacts/training_logs"
RUN_SUFFIX="${RUN_SUFFIX:-v5}"
