#!/usr/bin/env python3
"""
Polar gauge reader v4 — sin/cos regression with strong augmentation.

Key differences from v1:
1. Proper data augmentation: color jitter, noise, vertical shift (radius perturbation).
   These preserve the needle angle while varying appearance.
2. Higher dropout + L2 regularization for overfitting control.
3. Vertical polar shift (radius axis) — simulates different crop centers.
   Needle angle is UNCHANGED (horizontal dimension = angle).
"""
...
class Config:
    ...
    add_edges: bool = True
    experiment: str = "exp1"
    output_dir: str = "/tmp/gauge_polar_v4"
...
def augment_polar(image, label):
    """Color jitter (on RGB channels only) + noise."""
    rgb = image[..., :3]
...
def main():
    ...
    ap.add_argument("--output-dir", default="/tmp/gauge_polar_v4")
    args = ap.parse_args()
    ...
    print("=" * 60)
    print(f"POLAR GAUGE READER v4 — backbone={conf.backbone}")
    print(f"  sin/cos regression + strong augmentation")
