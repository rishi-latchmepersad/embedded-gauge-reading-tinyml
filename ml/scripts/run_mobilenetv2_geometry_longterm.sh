...
# Train the long-term MobileNetV2 geometry model with an explicit train/val/test
# split:
...
LOG_FILE="${LOG_DIR}/mobilenetv2_geometry_longterm.log"
BASE_MODEL_SRC="${REPO_ROOT}/artifacts/training/mobilenetv2_geometry_detector/model.keras"
BASE_MODEL_LOCAL="${HOME}/ml_eval_cache/mobilenetv2_geometry_longterm.model.keras"
...
echo "[WRAPPER] Starting long-term MobileNetV2 geometry fine-tune."
...
"${POETRY_BIN}" run python -u scripts/run_training.py \
  ...
  --run-name mobilenetv2_geometry_longterm \
  2>&1 | tee "${LOG_FILE}"
