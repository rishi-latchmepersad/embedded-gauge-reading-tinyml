"""Package the MobileNetV2 OBB+Center joint model for STM32N6 (no HyperRAM).

Single-model replacement for the two-stage OBB + center-detector pipeline.
Outputs 8 int8 values: [cx_obb, cy_obb, w, h, cos2t, sin2t, cx_center, cy_center].
...
MODEL_NAME: str = "obb_center_mnv2_int8"
PACKAGE_DIRNAME: str = "obb_center_mnv2_int8"
TFLITE_MODEL: Path = (
    REPO_ROOT
    / "ml"
    / "artifacts"
    / "training"
    / "obb_center_mnv2_20260612_062120"
    / "obb_center_mnv2_int8.tflite"
)
...
STAGING_BUILD: Path = REPO_ROOT / "tmp" / "stedgeai_obb_center_mnv2_build"
TEMP_WORKSPACE: Path = REPO_ROOT / "tmp" / "stedgeai_obb_center_mnv2_ws"
TEMP_OUTPUT: Path = REPO_ROOT / "tmp" / "stedgeai_obb_center_mnv2_out"
...
def main() -> None:
    ...
    # Copy raw to workspace dir for Cube.AI reference
