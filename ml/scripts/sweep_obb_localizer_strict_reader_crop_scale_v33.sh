...
# Sweep a tighter band of OBB crop scales around the best-known value.
# This keeps the localizer fixed and only tunes how much context the reader sees.

REPO_ROOT="/mnt/d/Projects/embedded-gauge-reading-tinyml/ml"
LOG_DIR="${REPO_ROOT}/artifacts/training_logs"
LOG_FILE="${LOG_DIR}/obb_localizer_strict_reader_crop_scale_sweep_v33.log"
...
SCALES=(1.28 1.29 1.30 1.31 1.32)
MIN_CROP_SIZE=48.0

echo "[SWEEP] Starting tighter OBB crop-scale sweep." | tee -a "${LOG_FILE}"
...
for scale in "${SCALES[@]}"; do
  OUT_DIR="${REPO_ROOT}/artifacts/cascade_eval/obb_localizer_strict_reader_v33_scale_${scale}"
