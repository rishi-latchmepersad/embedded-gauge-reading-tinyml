...
# Warm-start from the best board30 scalar checkpoint, but keep the data mix
# more balanced so we improve the whole hard-case set instead of overfitting
# a single outlier.
REPO_ROOT="/mnt/d/Projects/embedded-gauge-reading-tinyml/ml"
LOG_DIR="${REPO_ROOT}/artifacts/training_logs"
LOG_FILE="${LOG_DIR}/scalar_full_finetune_from_best_board30_balanced.log"
...
echo "[WRAPPER] Starting balanced scalar MobileNetV2 fine-tune."
...
"${POETRY_BIN}" run python -u scripts/run_training.py \
  ...
  --run-name scalar_full_finetune_from_best_board30_balanced \
  2>&1 | tee "${LOG_FILE}"
