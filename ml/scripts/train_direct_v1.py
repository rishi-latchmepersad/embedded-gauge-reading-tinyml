#!/usr/bin/env python3
"""
Direct gauge reader — simple CNN regression on 224×224 crops (no polar).

Idea: skip polar projection entirely. Train a simple CNN to regress sin/cos
directly from the preprocessed crop. This preserves ALL image information
(no polar projection artifacts) and lets the model learn gauge features naturally.

Compared to polar approach:
+ Full image information preserved
+ No projection artifacts / wrapping issues
+ Simpler pipeline
- Needle is curved (arc), not a straight line
- Background variation is higher

Compared to MobileNetV2 scalar baseline (12.5°C):
+ Much fewer params → less overfitting
+ Train from scratch → no pretrained BN mismatch
"""
...
class Config:
    ...
    dropout: float = 0.25
    experiment: str = "exp1"
    output_dir: str = "/tmp/gauge_direct_v1"
...
def main():
    ...
    ap.add_argument("--output-dir", default="/tmp/gauge_direct_v1")
    args = ap.parse_args()
    ...
    print("=" * 60)
    print(f"DIRECT GAUGE READER v1 — simple CNN on 224×224 crops")
    print(f"  sin/cos regression (no polar projection)")
    ...
    cb = [
        keras.callbacks.EarlyStopping(monitor="val_sort_fn", patience=20, restore_best_weights=True, mode="min"),
        keras.callbacks.ReduceLROnPlateau(monitor="val_sort_fn", factor=0.5, patience=10, min_lr=1e-6, mode="min"),
    ]
