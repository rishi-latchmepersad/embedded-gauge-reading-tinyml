"""Compare board-matching classical baseline vs OBB+center detector pipeline."""
...
parser.add_argument("--obb", type=str,
    default=str(ARTIFACTS / "obb_improved_20260530_194719" / "model.keras"))
parser.add_argument("--ctr", type=str,
    default=str(ARTIFACTS / "center_detector_v1_20260530_201137" / "best_model.keras"))
...
examples, _ = _build_training_examples(samples, spec,
    image_height=H, image_width=W,
    keypoint_heatmap_size=28, strict_labels=False, crop_pad_ratio=0.25)
config = TrainConfig(gauge_id="littlegood_home_temp_gauge_c",
...
for ex in test_examples:
    img, _ = _load_crop_and_preprocess_image(ex.image_path, 0.0,
        ex.crop_box_xyxy, H, W)
    imgs.append(img.numpy())
    gts.append(math.degrees(
        math.atan2(ex.needle_unit_xy[1], ex.needle_unit_xy[0])) % 360)
    img_names.append(Path(ex.image_path).name)
x_test = np.array(imgs)

print("[Eval] Loading models...")
obb = keras.models.load_model(str(OBB_PATH), compile=False,
    custom_objects=CUSTOM_OBJECTS)
ctr = keras.models.load_model(str(CTR_PATH), compile=False,
    custom_objects=CUSTOM_OBJECTS)
...
for i, ex in enumerate(test_examples):
    ...
    gt_center = ex.center_xy

    img_uint8 = (x_test[i] * 255).astype(np.uint8) if x_test[i].max() <= 1.0 else x_test[i].astype(np.uint8)
    img_bgr = img_uint8[..., ::-1].copy()

    # Firmware-matched baseline (run on same crop as other approaches)
    fw_cx, fw_cy, fw_radius, fw_det = run_firmware_baseline(img_bgr, spec)

    # OBB center + polar
    obb_res = _detect_needle_unit_vector_polar(img_bgr,
        center_xy=tuple(obb_center), dial_radius_px=dial_radius_px, gauge_spec=spec)
    # Center detector + polar
    ctr_res = _detect_needle_unit_vector_polar(img_bgr,
        center_xy=tuple(ctr_center), dial_radius_px=dial_radius_px, gauge_spec=spec)
    # GT center + polar (oracle)
    gt_res = _detect_needle_unit_vector_polar(img_bgr,
        center_xy=tuple(gt_center), dial_radius_px=dial_radius_px, gauge_spec=spec)

    def get_angle(res):
        if res is None: return None
        return math.degrees(math.atan2(res.unit_dy, res.unit_dx)) % 360

    fw_angle = get_angle(fw_det)
    ...
    fw_err = err_or_none(fw_angle, gt_deg)
    obb_err = err_or_none(obb_angle, gt_deg)
    ctr_err = err_or_none(ctr_angle, gt_deg)
    gt_err = err_or_none(gt_angle, gt_deg)
    fw_center_dist = np.linalg.norm(np.array([fw_cx, fw_cy]) - gt_center)
    obb_center_dist = np.linalg.norm(obb_center - gt_center)
    ctr_center_dist = np.linalg.norm(ctr_center - gt_center)

    results.append((img_names[i], gt_deg,
        fw_angle, obb_angle, ctr_angle, gt_angle,
        fw_err, obb_err, ctr_err, gt_err,
        fw_center_dist, obb_center_dist, ctr_center_dist))

print(f"\n{'='*140}")
h = f"{'File':36s} {'GT°':6s} {'FW°':6s} {'OBB°':6s} {'CTR°':6s} {'GT°':7s} {'dFW':6s} {'dOBB':6s} {'dCTR':6s}"
print(h)
print("-" * 140)
for r in results:
    fname, gt, fwa, oba, cta, gta, fwe, obe, cte, gte, fwd, obd, ctd = r
    def fs(v, fmt="6.1f"):
        return f"{v:{fmt}}" if v is not None else "  FAIL"
    print(f"{fname:36s} {gt:6.1f} {fs(fwa)} {fs(oba)} {fs(cta)} {fs(gta, '7.1f')} {fwd:6.2f} {obd:6.2f} {ctd:6.2f}")

def clean(arr):
    return np.array([v for v in arr if v is not None], dtype=float)

fw_errs = clean([r[6] for r in results])
obb_errs = clean([r[7] for r in results])
ctr_errs = clean([r[8] for r in results])
gt_errs = clean([r[9] for r in results])
fw_dists = np.array([r[10] for r in results])
obb_dists = np.array([r[11] for r in results])
ctr_dists = np.array([r[12] for r in results])
...
def summary(name, errs):
    ...
    return (f"{name:40s} MAE={errs.mean():6.2f}° "
            f"median={np.median(errs):6.2f}° "
            f"rmse={np.sqrt((errs**2).mean()):6.2f}°  "
            f"≤2°={in2:.0f}%  ≤5°={in5:.0f}%  ≤10°={in10:.0f}% (n={len(errs)})")

print(f"\n{'='*140}")
print(summary("Firmware baseline (polar)", fw_errs))
print(summary("OBB center (polar)", obb_errs))
print(summary("Center detector (polar)", ctr_errs))
print(summary("GT oracle (polar)", gt_errs))
print(f"\nCenter distances:")
print(f"  Firmware: {fw_dists.mean():.2f}±{fw_dists.std():.2f}px (n={len(fw_dists)})")
print(f"  OBB:      {obb_dists.mean():.2f}±{obb_dists.std():.2f}px")
print(f"  Center:   {ctr_dists.mean():.2f}±{ctr_dists.std():.2f}px")
