#!/usr/bin/env python3
"""Train on all data with frozen backbone only — head-only training.

The comprehensive scripts failed because fine-tuning the backbone with 141 images
causes immediate overfitting and model collapse. This script trains ONLY the head
for 40 epochs with a frozen pretrained backbone, using MSE loss for stronger
penalty on large errors.

Usage:
    cd /mnt/d/Projects/embedded-gauge-reading-tinyml/ml
    poetry run python scripts/train_head_only.py \
        --output-dir artifacts/training/head_only
"""
...
def train_head_only(
    ...
    """Train with frozen backbone only — no fine-tuning."""
    np.random.seed(seed)
    ...
    model = build_mobilenetv2_regression_model(
        ...
        alpha=alpha,
        head_units=256,  # Increased from 128
    ...
    logger.info("Backbone FROZEN for entire training")

    # Compile with MSE loss (stronger penalty for large errors)
    model.compile(
    ...
    # Train
    logger.info(f"\n=== Training head only ({epochs} epochs, frozen backbone) ===")
    history = model.fit(
        train_ds,
