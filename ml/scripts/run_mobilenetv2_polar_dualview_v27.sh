...
# Train the new polar-dualview MobileNetV2 regression model on all real sources
# plus corrected synthetic gauge renders.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOG_DIR="${ROOT_DIR}/artifacts/training_logs"
LOG_FILE="${LOG_DIR}/mobilenetv2_polar_dualview_v27.log"
...
TMP_DIR="../tmp/polar_dualview_v27"
...
COMBINED_MANIFEST="${TMP_DIR}/combined_polar_dualview_manifest_v27.csv"
...
OUTPUT_DIR="artifacts/training/mobilenetv2_polar_dualview_v27"
HARD_CASE_MANIFEST="data/hard_cases_plus_board30_valid_with_new5.csv"

echo "[WRAPPER] Starting polar-dualview v27." | tee "${LOG_FILE}"
...
echo "[WRAPPER] Building the combined polar-dualview manifest..." | tee -a "${LOG_FILE}"
...
echo "[WRAPPER] Training the new polar-dualview model..." | tee -a "${LOG_FILE}"
