"""Train QAT-safe ellipse detector with multi-head output for better radius prediction.

Key improvement over v1: separate center and radius output heads with
different loss weights.  The radius head gets 3x the loss weight of the
center head, preventing the model from learning a constant radius.

Architecture: QAT-safe encoder (Conv+BN+ReLU) + three separate heads:
  - center_xy: Dense(2, sigmoid) for [cx, cy]
  - radius_xy: Dense(2, sigmoid) for [rx, ry] (3x loss weight)
  - confidence: Dense(1, sigmoid) for confidence
"""
...
def build_model(width_multiplier: float = 1.5) -> keras.Model:
    """QAT-safe ellipse regressor with multi-head output.

    Separate heads for center and radius allow different loss weights,
    preventing the radius from collapsing to a constant.
    """
    ...
    # Radius head: [rx, ry] — separate to allow higher loss weight
    radius = keras.layers.Dense(64, activation="relu", name="radius_dense")(shared)
    radius_xy = keras.layers.Dense(2, activation="sigmoid", name="radius_xy")(radius)
    ...
    return keras.Model(
        ...
        name=f"ellipse_qat_multihead_w{wm}",
    )
...
def _build_dataset(paths, targets, batch, training, weights=None):
    ...
    def _decode(path, target, weight):
        ...
        # Split target into center, radius, confidence
        center = target[:2]
        radius = target[2:4]
        conf = target[4:5]
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path(__file__).resolve().parents[1] / "artifacts" / "gauge_ellipse_qat_multihead_v1")
    ...
    # Multi-head loss: radius gets 3x weight
    losses = [
        keras.losses.Huber(delta=0.05),  # center
        keras.losses.Huber(delta=0.05),  # radius
        keras.losses.Huber(delta=0.05),  # confidence
    ]
    loss_weights = [1.0, 3.0, 0.1]  # radius gets 3x, confidence gets 0.1x
    ...
    # -- Export --
    tflite_path = args.output / "ellipse_qat_multihead_int8.tflite"
    ...
    report = {
        "model": f"ellipse_qat_multihead_w{args.width}",
