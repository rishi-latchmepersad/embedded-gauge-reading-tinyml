...
#
# This version keeps the same strict rectified pool, but uses a preview-heavy
# augmentation profile so the model sees more low-contrast, grayscale, and
# slightly blurred crops during the real fine-tune stage.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOG_DIR="${ROOT_DIR}/artifacts/training_logs"
LOG_FILE="${LOG_DIR}/mobilenetv2_rectified_scalar_from_hard_synth_previewaug_v14.log"
...
OUTPUT_DIR="artifacts/training/mobilenetv2_rectified_scalar_from_hard_synth_previewaug_v14"

echo "[WRAPPER] Starting rectified fine-tune from hard synthetic pretrain v14." | tee "${LOG_FILE}"
