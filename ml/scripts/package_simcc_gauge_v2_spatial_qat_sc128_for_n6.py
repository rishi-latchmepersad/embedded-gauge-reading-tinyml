"""Package the spatial SimCC gauge model for STM32N6 deployment.

This helper converts the saved SimCC PTQ TFLite artifact into the relocatable
ST AI package that the firmware wrapper includes at build time.
"""
...
MODEL_NAME: str = "simcc_gauge_v2_spatial_qat_sc128_int8"
PACKAGE_DIRNAME: str = "simcc_gauge_v2_spatial_qat_sc128_int8_n6_npu"
TFLITE_MODEL: Path = (
    REPO_ROOT
    / "ml"
    / "artifacts"
...
STAGING_BUILD: Path = REPO_ROOT / "tmp" / "stedgeai_simcc_sc128_build"
TEMP_WORKSPACE: Path = REPO_ROOT / "tmp" / "stedgeai_simcc_sc128_ws"
TEMP_OUTPUT: Path = REPO_ROOT / "tmp" / "stedgeai_simcc_sc128_out"
...
def _print_signature(raw_path: Path) -> None:
    """Print the xSPI2 prefix and suffix so flash checks can be updated."""
...
def main() -> None:
    ...
    print(
        ...
        f"({generated_raw.stat().st_size} bytes)",
        flush=True,
    )
    ...
    for src in TEMP_OUTPUT.glob("network*"):
        if src.is_file():
            dst = STAI_OUTPUT_DIR / src.name
            shutil.copy2(src, dst)
            print(f"[PACKAGE]   {src.name} -> {dst}", flush=True)
    ...
    for src_obj in STAGING_BUILD.glob("*.o"):
        shutil.copy2(src_obj, build_net_dir / src_obj.name)
