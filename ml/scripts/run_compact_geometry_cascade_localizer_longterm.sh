...
# Fine-tune the compact CNN geometry localizer as a board-pinned cascade front end.
#
# This keeps the model lightweight, warm-starts from the broad compact geometry
# checkpoint, and pins the validation/test manifests so the run is judged on the
# board-style split rather than only on the easy in-range training mix.
REPO_ROOT="/mnt/d/Projects/embedded-gauge-reading-tinyml/ml"
LOG_DIR="${REPO_ROOT}/artifacts/training_logs"
LOG_FILE="${LOG_DIR}/compact_geometry_cascade_localizer_longterm.log"
BASE_MODEL_SRC="${REPO_ROOT}/artifacts/training/compact_geometry_full_range/model.keras"
BASE_MODEL_LOCAL="${HOME}/ml_eval_cache/compact_geometry_cascade_localizer_longterm.model.keras"
...
echo "[WRAPPER] Starting compact geometry cascade localizer long-term fine-tune."
...
"${POETRY_BIN}" run python -u scripts/run_training.py \
  ...
  --run-name compact_geometry_cascade_localizer_longterm \
  2>&1 | tee "${LOG_FILE}"
