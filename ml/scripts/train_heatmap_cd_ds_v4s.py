"""
DS-CNN v4-S (Slim): 160x160 heatmap, halved channels, fits on-chip SRAM.

Variant of v4 redesigned for boards without hyperRAM. The 50% channel
reduction cuts peak activation memory by ~60%, targeting <2.5 MB total
to fit the 2.82 MB AXISRAM2-6 NPU pool.

Architecture (vs v4 in brackets):
  - Encoder: 32->64->96->192  [v4: 48->96->192->384]
  - Decoder: 192->96->64->32  [v4: 384->160->96->48]
  - Output: 160x160x1 heatmap (same as v4)

Everything else unchanged: CoordConv, ReLU6, bilinear upsampling,
skip connections at 40x40 and 80x80, sigma=6.0, MSE loss.

Usage:
  cd ml && poetry run python scripts/train_heatmap_cd_ds_v4s.py
"""
...
ARTIFACT_DIR = Path(__file__).resolve().parents[1] / "artifacts" / "heatmap_cd_ds_v4s"
METADATA_PATH = DATA_DIR / "metadata.json"

# Slim: halved channel counts relative to v4 (48->96->192->384 / 384->160->96->48)
ENC_HEAD = 32    # was 48
ENC_S1 = 64      # was 96
ENC_S2 = 96      # was 192
ENC_S3 = 192     # was 384
...
def add_coordconv(x: tf.Tensor) -> tf.Tensor:
    """Append two normalized [-1,1] coordinate channels for spatial awareness."""
    _, h, w, _ = x.shape
...
def build_model(
    ...
    """DS-CNN v4-S with halved channels for on-chip-only SRAM deployment.

    Encoder: 320->160->80->40->20 (channels 32->64->96->192)
    Decoder: 20->40->80->160 (channels 192->96->64->32)
    Output: 160x160x1 sigmoid heatmap.
    """
    ...
    # Head conv: 5 -> ENC_HEAD, stride 2
    x = layers.Conv2D(
        ENC_HEAD, 3, strides=2, padding="same", use_bias=False, name="enc_head"
    )(x)
    x = layers.BatchNormalization(name="enc_head_bn")(x)
    x = layers.ReLU(6.0, name="enc_head_relu6")(x)  # 160x160x32

    # Encoder stages
    x = ds_conv_block(x, ENC_S1, stride=2, name="enc_s1")  # 80x80x64
    skips["s1"] = x

    x = ds_conv_block(x, ENC_S2, stride=2, name="enc_s2")  # 40x40x96
    skips["s2"] = x

    x = ds_conv_block(x, ENC_S3, stride=2, name="enc_s3")  # 20x20x192

    # Decoder
    x = layers.UpSampling2D(2, interpolation="bilinear", name="dec_up1")(x)  # 40x40
    x = layers.Concatenate(name="dec_cat1")([x, skips["s2"]])
    x = ds_conv_block(x, DEC_DS1, stride=1, name="dec_ds1")  # 40x40x96

    x = layers.UpSampling2D(2, interpolation="bilinear", name="dec_up2")(x)  # 80x80
    x = layers.Concatenate(name="dec_cat2")([x, skips["s1"]])
    x = ds_conv_block(x, DEC_DS2, stride=1, name="dec_ds2")  # 80x80x64

    x = layers.UpSampling2D(2, interpolation="bilinear", name="dec_up3")(x)  # 160x160
    x = ds_conv_block(x, DEC_DS3, stride=1, name="dec_ds3")  # 160x160x32
...
def heatmap_from_center(
    ...
    """Generate Gaussian heatmap at the specified resolution."""
...
def augment_image(img: tf.Tensor, hm: tf.Tensor) -> tuple[tf.Tensor, tf.Tensor]:
    """Mild photometric augmentation, same as v4."""
...
def center_pixel_error(
    ...
    """Compute mean and median center error using soft-argmax on heatmaps."""
    errors = []
...
def eval_tflite(
    ...
    in_scale_val, in_zero = (
        in_d["quantization"] if is_int8 else (1.0, 0)
    )
    out_scale_val, out_zero = (
        out_d["quantization"] if is_int8 else (1.0, 0)
    )

    errs = []
    for i in range(len(X_val)):
        if is_int8:
            # X_val is in [-1, 1], quantize to uint8
            inp_uint8 = np.clip(
...
def main() -> None:
    ...
    def load_split(
        ...
        """Load JPEG images and generate 160x160 heatmap targets."""
    ...
    print(
        f"\nChannel config (v4-S / v4 original):"
        f"\n  Enc: {ENC_HEAD}/{ENC_S1}/{ENC_S2}/{ENC_S3}  "
        f"(was 48/96/192/384)"
        f"\n  Dec: {DEC_DS1}/{DEC_DS2}/{DEC_DS3}  "
        f"(was 160/96/48)"
    )
    ...
    print(
        f"\nKeras -- Val center error ({HEATMAP_SIZE}x{HEATMAP_SIZE}): "
    ...
    results = {
        "model": "depthwise_separable_cnn_v4s_slim_160",
        "params": trainable,
        ...
        "channels_dec": [DEC_DS1, DEC_DS2, DEC_DS3],
    ...
    # --- Sub-pixel refinement eval (argmax + parabola) ---
    print("\n--- Sub-pixel refinement (argmax + 1D parabola) ---")

    def refine_center(heatmap: np.ndarray) -> tuple[float, float]:
        """Argmax + 1D parabolic sub-pixel refinement on 160x160 heatmap."""
        h, w = heatmap.shape
    ...
    in_quant = interp_int8.get_input_details()[0]["quantization"]
    out_quant = interp_int8.get_output_details()[0]["quantization"]
    in_scale_val_sp, in_zero_sp = in_quant
    out_scale_val_sp, out_zero_sp = out_quant

    refine_errs = []
    for i in range(len(X_val)):
        inp_uint8 = np.clip(
            np.round(X_val[i : i + 1] / in_scale_val_sp + in_zero_sp),
            0, 255,
        ).astype(np.uint8)
        ...
        pred = (raw.astype(np.float32) - out_zero_sp) * out_scale_val_sp
        pred_hm = pred[0, :, :, 0]
        ref_r, ref_c = refine_center(pred_hm)
        gt_pt = softargmax_2d(y_val[i, :, :, 0])
        dist = np.sqrt((gt_pt[0] - ref_r) ** 2 + (gt_pt[1] - ref_c) ** 2)
