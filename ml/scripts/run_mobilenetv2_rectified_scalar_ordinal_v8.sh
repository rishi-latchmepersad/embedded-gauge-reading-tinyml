...
# Train a rectified MobileNetV2 with an ordinal auxiliary head so the network
# must learn coarse temperature ordering, not just the middle of the range.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOG_DIR="${ROOT_DIR}/artifacts/training_logs"
LOG_FILE="${LOG_DIR}/mobilenetv2_rectified_scalar_ordinal_v8.log"
...
echo "[WRAPPER] Starting ordinal rectified scalar fine-tune v8." | tee "${LOG_FILE}"
...
echo "[WRAPPER] Launching ordinal training..." | tee -a "${LOG_FILE}"
"${POETRY_BIN}" run python -u scripts/train_all_data_baseline.py \
  --output-dir artifacts/training/mobilenetv2_rectified_scalar_ordinal_v8 \
  ...
  --alpha 0.35 \
  --head-units 64 \
  --aux-head-kind ordinal \
  --ordinal-threshold-step 10.0 \
