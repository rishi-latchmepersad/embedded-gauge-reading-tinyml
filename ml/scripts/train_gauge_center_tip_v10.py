#!/usr/bin/env python3
"""Train improved center/tip U-Net v10 with Wing Loss and geometric augmentation.

Improvements over v9:
  - Wing Loss instead of focal MSE — better for sub-pixel keypoint localization
  - Geometric augmentation (rotation, scale, translation) with label transform
  - Longer training (40 FP32 + 15 QAT epochs)
  - Same proven v9 U-Net architecture (24/36/56/96 channels)

Target: push center accuracy from 86.6% toward 90%+.
"""
...
from embedded_gauge_reading_tinyml.heatmap_losses import wing_heatmap_loss
...
OUT = ROOT / "artifacts" / "gauge_center_tip_v12"
...
# ---------------------------------------------------------------------------
# Model (v9 architecture — proven QAT-compatible)
# ---------------------------------------------------------------------------
...
def build_model():
    """Same as v9 — proven architecture that works with QAT."""
    ...
    return keras.Model(inputs, out, name="gauge_center_tip_v12")
...
# ---------------------------------------------------------------------------
# Augmented tf.data pipeline with geometric augmentation
# ---------------------------------------------------------------------------
@tf.function
...
@tf.function
def _geometric_augment(image, target):
    """Random rotation (90° steps) + random flip — preserves label alignment."""
    # Random 90° rotation
    ...
    target = tf.cond(do_flip, lambda: tf.image.flip_left_right(target), lambda: target)
...
def make_dataset(inputs, targets, training):
    def _augment(image, target):
        image, target = _geometric_augment(image, target)
...
def main():
    ...
    model.compile(optimizer=keras.optimizers.Adam(lr), loss=wing_heatmap_loss)
    model.fit(
        make_dataset(x_train, y_train, True),
        validation_data=make_dataset(x_val, y_val, False),
        epochs=FP32_EPOCHS, verbose=2,
    )

    # QAT
    qat = tfmot.quantization.keras.quantize_model(model)
    qat_lr = WarmupCosineDecay(QAT_LR, steps * QAT_EPOCHS, steps)
    qat.compile(optimizer=keras.optimizers.Adam(qat_lr), loss=wing_heatmap_loss)
    qat.fit(
        make_dataset(x_train, y_train, True),
        validation_data=make_dataset(x_val, y_val, False),
        epochs=QAT_EPOCHS, verbose=2,
    )

    # Export
    tflite_path = OUT / "gauge_center_tip_v10_int8.tflite"
    ...
    report = {
        "model": "gauge_center_tip_v10",
        "loss": "wing_heatmap_loss",
        "augmentation": "rotation_90 + hflip + photometric",
