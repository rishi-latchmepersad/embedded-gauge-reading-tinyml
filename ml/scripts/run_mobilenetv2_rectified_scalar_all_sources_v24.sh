...
# Recreate the strict v5 rectified scalar model on the broadest usable data mix.
#
# This keeps the proven v5 architecture and training policy, but expands the
# manifest to include the full CVAT-labelled pool, the broader scalar pool, and
# a small amount of corrected synthetic data for geometric regularization.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOG_DIR="${ROOT_DIR}/artifacts/training_logs"
LOG_FILE="${LOG_DIR}/mobilenetv2_rectified_scalar_all_sources_v24.log"
...
TMP_DIR="../tmp"
SYNTH_DIR="${TMP_DIR}/synth_gauge_all_sources_v24"
SYNTH_MANIFEST="${SYNTH_DIR}/manifest.csv"
COMBINED_MANIFEST="${TMP_DIR}/combined_scalar_manifest_v24.csv"
...
OUTPUT_DIR="artifacts/training/mobilenetv2_rectified_scalar_all_sources_v24"
HARD_CASE_MANIFEST="data/hard_cases_plus_board30_valid_with_new5.csv"

LOG_DIR_REL="artifacts/training_logs"

echo "[WRAPPER] Starting strict v5 all-sources fine-tune v24." | tee "${LOG_FILE}"
...
echo "[WRAPPER] Fine-tuning strict v5 on all sources..." | tee -a "${LOG_FILE}"
