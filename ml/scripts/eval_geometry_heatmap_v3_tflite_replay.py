#!/usr/bin/env python3
"""Replay geometry_heatmap_v3 through Keras and TFLite on one split.
...
DEFAULT_OUTPUT_DIR = Path("artifacts/deployment/geometry_heatmap_v3_tflite")
DEFAULT_PREDICTIONS_PATH = Path("artifacts/deployment/geometry_heatmap_v3_tflite/v3_tflite_replay_predictions.csv")
DEFAULT_SUMMARY_PATH = Path("artifacts/deployment/geometry_heatmap_v3_tflite/v3_tflite_replay_summary.csv")
DEFAULT_REMAINING_PATH = Path("artifacts/deployment/geometry_heatmap_v3_tflite/v3_remaining_worst_accepted.csv")
DEFAULT_REPORT_PATH = Path("ml/reports/geometry_heatmap_v3_tflite_replay.md")
DEFAULT_KERAS_VALIDATION_REPORT_PATH = Path("ml/reports/geometry_heatmap_v3_keras_validation.md")
...
def main() -> None:
    """Replay geometry_heatmap_v3 on one split and save evaluation artifacts."""

    parser = argparse.ArgumentParser(description="Replay geometry_heatmap_v3 TFLite exports")
    ...
    print(f"[V3 REPLAY] Split: {args.split}", flush=True)
    print(f"[V3 REPLAY] Keras accepted MAE: {keras_summary['accepted_mae_c']:.4f} C", flush=True)
    print(f"[V3 REPLAY] Keras acceptance rate: {keras_summary['acceptance_rate']:.4f}", flush=True)
    print(f"[V3 REPLAY] Keras worst accepted error: {keras_summary['worst_accepted_error_c']:.4f} C", flush=True)
    print(f"[V3 REPLAY] INT8 accepted MAE: {int8_summary['accepted_mae_c']:.4f} C", flush=True)
    print(f"[V3 REPLAY] INT8 acceptance rate: {int8_summary['acceptance_rate']:.4f}", flush=True)
    print(f"[V3 REPLAY] INT8 worst accepted error: {int8_summary['worst_accepted_error_c']:.4f} C", flush=True)
    print(
        f"[V3 REPLAY] INT8 temp drift mean/median/p90: "
        f"{int8_drift['temperature_delta_mean']:.4f} / {int8_drift['temperature_delta_median']:.4f} / {int8_drift['temperature_delta_p90']:.4f}",
        flush=True,
    )
    print(
        f"[V3 REPLAY] INT8 tip drift mean/median: {int8_drift['tip_delta_mean']:.4f} / {int8_drift['tip_delta_median']:.4f}",
        flush=True,
    )
    print(f"[V3 REPLAY] Guardrail disagreements: {int(int8_drift['guardrail_disagreements'])}", flush=True)
    print(f"[V3 REPLAY] Tensor contract: {contract_path}", flush=True)
