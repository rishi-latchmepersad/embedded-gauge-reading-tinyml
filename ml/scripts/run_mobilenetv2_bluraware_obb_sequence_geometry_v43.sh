...
# Train a blur-aware OBB sequence-geometry MobileNetV2 variant and replay it
# against the hard-case manifest with the fixed board front-end.
# The reader now stays geometry-first: OBB crop, sharpened/raw fusion, pointer
# mask, four keypoints, and a deterministic value head on top.

REPO_ROOT="/mnt/d/Projects/embedded-gauge-reading-tinyml/ml"
POETRY_BIN="${POETRY_BIN:-${HOME}/.local/bin/poetry}"
LOG_DIR="${REPO_ROOT}/artifacts/training_logs"
TRAIN_LOG_FILE="${LOG_DIR}/mobilenetv2_bluraware_obb_sequence_geometry_v43.log"
EVAL_LOG_FILE="${LOG_DIR}/mobilenetv2_bluraware_obb_sequence_geometry_v43_hardcase.log"
RUN_NAME="${RUN_NAME:-mobilenetv2_bluraware_obb_sequence_geometry_v43}"
...
INIT_MODEL_SRC=""
for candidate in \
  "${REPO_ROOT}/artifacts/training/mobilenetv2_bluraware_obb_relation_geometry_v42/model.keras" \
...
echo "[WRAPPER] Starting blur-aware OBB-sequence-geometry training." | tee "${TRAIN_LOG_FILE}"
