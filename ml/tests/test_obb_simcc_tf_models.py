"""Tests for the combined OBB + SimCC gauge model builders."""
...
from embedded_gauge_reading_tinyml.obb_simcc_tf_models import (
    build_mobilenetv2_obb_simcc_model,
)
...
def test_build_mobilenetv2_obb_simcc_model_outputs_expected_heads() -> None:
    """The combined model should emit the OBB, SimCC, and optional temp heads."""

    model = build_mobilenetv2_obb_simcc_model(
    ...
    assert model.name == "mobilenetv2_obb_simcc_gauge"
    ...
    assert model.get_layer("obb_center_xy").__class__.__name__ == "Dense"
...
def test_build_mobilenetv2_obb_simcc_model_is_qat_cloneable() -> None:
    """The combined model should be compatible with tfmot quantization."""
    ...
    if isinstance(outputs, dict):
        assert tuple(outputs["obb_center_xy"].shape) == (2, 2)
        assert tuple(outputs["obb_size_wh"].shape) == (2, 2)
        assert tuple(outputs["obb_angle_sincos"].shape) == (2, 2)
